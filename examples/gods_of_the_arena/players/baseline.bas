' Gods of the Arena bundled human-play teammates and opponents.
'
' Same economy as base.bas. Answers one team call with attackMove, and
' may spend a primary charge on the nearest visible enemy.
'
' Extra host data for calls: allyPingX, allyPingY, allyPingKind, allyPingBy.
' Extra action: attackMove(x, y).

decisions = decisions + 1

if allyPingBy <> 0 then
  if allyPingBy <> selfId then
    attackMove(allyPingX, allyPingY)
  end if
else
  bestId = 0
  ...
  if bestId <> 0 then
    if abilityCharges(1) > 0 then
      if abilityCooldown(1) = 0 then
        castTarget(1, bestId)
      end if
    end if
    attackTarget(bestId)
