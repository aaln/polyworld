...
def run(directory, workers, family='economy'):
    ...
    if plan.get('selection_mode') == 'win_first':
        for name in metrics:
            rr = [r for r in rows if r['name'] == name]
            metrics[name].update(deaths=sum(r['deaths'] for r in rr),
                mean_deaths=sum(r['deaths'] for r in rr)/len(rr),
                mean_minutes=sum(r['ticks'] for r in rr)/1440/len(rr),
                xp_per_minute=sum(r['total_xp'] for r in rr)*1440/sum(r['ticks'] for r in rr))
        control = metrics[plan['survival_control']]
        eligible = [n for n in plan['variants'] if
            all(metrics[n]['wins'] >= metrics[c]['wins'] + delta for c,delta in plan['win_thresholds'].items()) and
            metrics[n]['mean_deaths'] <= 1.1 * control['mean_deaths'] and
        ...
        eligible = [n for n in plan['variants'] if
                    all(metrics[n]['wins'] >= metrics[c]['wins'] + delta for c, delta in plan.get('win_thresholds', {c: 2 for c in plan.get('selection_controls', ['v2', 'default'])}).items()) and
                    (plan.get('family') == 'release' or metrics[n]['wins'] >= metrics['motion']['wins']) and
                    metrics[n]['death_rate'] <= 1.1 * metrics[plan.get('survival_control', 'v2')]['death_rate'] and
                    metrics[n]['xp'] >= .8 * metrics[plan.get('survival_control', 'v2')]['xp'] and
                    metrics[n]['gear_games'] == 40 and not metrics[n]['unverified_retreats'] and
                    metrics[n]['max_work'] <= 50000 and metrics[n]['max_instructions'] <= 20000]
