...
def run(directory, command, dry_run=False):
    ...
    body = {'idempotency_key': 'gota-release-field-20260916-' + version[:8] + '-' + digest(verdict)[:10],
    ...
    prior_seeds = {r['seed'] for path in [root / 'result.json', directory / 'hosted-discovery/result.json', *[Path(p) for p in plan.get('excluded_discovery_results', [])]]
                   for arm in read(path)['arms'].values() for r in arm['rows']}
    for path in plan.get('excluded_rival_results', []):
        prior_seeds.update(r['seed'] for arm in read(path)['rivals'].values() for r in arm['rows'])
    for path in plan.get('excluded_field_results', []):
        prior_seeds.update(r['seed'] for r in read(path)['rows'])
