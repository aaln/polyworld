"""Broad rotated mixed rosters and fixed-team rush checks for the lineup repair.
...
ROOT=STUDY/'hosted/blue_repair/guardrails'
...
def prepare(_=None):
    ...
    if not read(winner/'jordan-result.json')['passed']:raise ValueError('Fresh Jordan80 must pass first')
    prior=read(BASE/'paired-guardrail/plan.json')
    own=read(winner/'uploaded-version.json');meta=read(winner/'upload-request.json')
    source=(STUDY/'local/candidates/blue_repair/policy.bas').read_bytes()
    with client() as c:
        buddy=clone_for_aaron(c,winner,meta,source,'Identical locally parity-checked and Jordan80-tested candidate; broad mixed guardrail pending. Inert registration only.')
    v={'optimizer':own['id'],'aaron':buddy['id']}
    ...
    plan.update(control_versions=prior['candidate_versions'],candidate_versions=[v['optimizer'],v['aaron']],
                ...
                'Separately160new fixed-team games versusRichard69andkhors1,40percolor, with '
    ...
    heads=[]
    for oldroot in (BASE/'league-sweep/head-to-head/rival_bdc35084',BASE/'hosted/long_three/matchups/khors'):
        ...
        p.update(policy_version=v['optimizer'],policy_label='aaron-gota-ir-perimeter-blue_repair-0916:v1',
...
def report():
    ...
    if not (ROOT/'feedback').exists():
        src=STUDY/'hosted/blue_repair/jordan-feedback'
...
if __name__=='__main__':
    ...
    while not (STUDY/'hosted/comparison.json').exists():
        pid=read(STUDY/'xp-process.json')['pid']
        try:os.kill(pid,0)
        except ProcessLookupError:raise RuntimeError('Jordan queue stopped before result')
        time.sleep(15)
    if not read(STUDY/'hosted/comparison.json')['selected']:
        print('Jordan failed; no broad guardrail requests.',flush=True)
    else:
        while not (BASE/'league-sweep/completed.json').exists() and read(STUDY/'queue-state.json')['stage']!='original_sweep_resumed':
            time.sleep(5)
        if (BASE/'league-sweep/completed.json').exists():run()
        else:main(prepare,run,'lineup_guardrails360',STUDY/'local',QUEUE)
