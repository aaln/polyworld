...
def main(definition=None):
    ...
    if not (d/"plan.json").exists():
        ...
        plan={"created_at":datetime.now(timezone.utc).isoformat(),"game_version":"2026.9.15.3",
              "hypothesis":definition["hypothesis"],"spell_cast_gate":bool(definition.get("spells")),
              ...
              "instruments":{str(p.relative_to(ROOT)):digest(p.read_bytes()) for p in
                             [HERE/n for n in ["spell_kiting_study.py","episode_spell_kiting.nim","kiting_contract.py","audit_current.nim","binding.py","policy_ir.py","independent_ir.py","hypothesis_study.py",*definition.get("instruments",[])]]},
              "screen":cases(definition["screen_seed"],40),"validation":cases(definition["validation_seed"],120),
              "screen_rule":"40 fresh balanced independent cases, candidate and v2 and default each vs nine defaults. Candidate wins >= each reference; deaths <=85% of v2, lifetime XP >=80% (spell damage can replace basics); spell-enabled cells need >=10 accepted explicit casts across >=5 games; movement in >=30% of cases, 100% normal retreats after a replay-observed hit within1tick; no corrected adverse class/regression flag. Directional screen only.",
    ...
    if plan["spell_cast_gate"] != bool(definition.get("spells")):raise ValueError("Frozen spell gate changed")
    ...
    for stage in [args.stage]:
        ...
        spell_casts=sum(h.get("kite_spell_casts") or 0 for h in hs)
        spell_games=sum((h.get("kite_spell_casts") or 0)>0 for h in hs)
        activation.update(spell_casts=spell_casts,spell_games=spell_games)
        if definition.get("spells"):
            activated=activated and spell_casts>=10 and spell_games>=5
        ...
        xp_ratio=metrics["candidate"]["xp"]/max(1,metrics["parent"]["xp"])
        gates={k:{"gain_wins":p["wins"]-p["control_wins"],"passed":p["wins"]>=p["control_wins"]} for k,p in pairs.items()}
        gates["survival"]={"death_ratio":death_ratio,"hit_ratio":hit_ratio,"xp_ratio":xp_ratio,"passed":death_ratio<=.85 and xp_ratio>=.8}
