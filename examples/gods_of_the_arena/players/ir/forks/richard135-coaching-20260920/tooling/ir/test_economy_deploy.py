...
from economy_deploy import AARON, CONTROL, DIVISION, MOTION, OPTIMIZER, owned_champions, readiness
...
class DeploymentBoundaryTests(unittest.TestCase):
    def setUp(self):
        ...
        files = {
            ...
            'hosted-confirmation/result.json': {'passed': True, 'selected': 'test',
                'arms': {n: {'games': 400} for n in ['v2', 'motion', 'test']}},
        ...
        source_path = self.root / 'economy/candidates/test/policy.bas'
    ...
    def test_missing_games_are_not_a_finished_confirmation(self):
        self.change('hosted-confirmation/result.json', {'arms': {n: {'games': 399} for n in ['v2', 'motion', 'test']}})
    ...
    def test_post_evaluation_code_edit_cannot_deploy(self):
        path = self.root / 'economy/candidates/test/policy.bas'
    ...
    def test_deployment_preserves_two_players_and_aarons_policy(self):
        rows = [{'player': {'id': player}, 'policy_version': {'id': version},
                ...
                for player, version in [(AARON, CONTROL), (OPTIMIZER, MOTION)]]
