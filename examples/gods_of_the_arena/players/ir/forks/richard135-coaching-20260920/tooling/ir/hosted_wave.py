...
def helper():
    spec = importlib.util.spec_from_file_location("eval_request", HELPER)
...
def client():
    return ResilientClient(base_url=API, headers={
        "Authorization": f"Bearer {load_current_token(server=get_api_server())}"},
...
def fetch(c, episode, directory, own_version, *, allow_repeated_subject=False):
    ...
    for kind, name in [("results", "results.json"), ("replay", "replay.bin"), ("logs", "game.log")]:
        ...
        if kind == 'logs' and response.status_code == 404:
            missing = response.json()
            if str(missing.get('detail', '')).startswith('No logs found for job '):
                # Preserve the actual platform response; never manufacture a log.
                write(folder / 'game-log-response.json', {
                    'episode_id': episode['id'], 'status_code': 404,
                    'path': f'{base}/artifacts/logs', 'body': missing})
    ...
    if (folder / 'game.log').exists() and capture_failed((folder / 'game.log').read_text()):
    ...
    if roster.count(own_version) == 0 or (not allow_repeated_subject and roster.count(own_version) != 1):
        raise ValueError("Subject must occupy exactly one seat")
    for slot, version in enumerate(roster):
        if version != own_version:
            continue
        response = c.get(f"{base}/{own_version}/policy-logs/{slot}")
        if response.status_code == 200:
            name = f'own-policy-slot-{slot}.log' if allow_repeated_subject else 'own-policy.log'
