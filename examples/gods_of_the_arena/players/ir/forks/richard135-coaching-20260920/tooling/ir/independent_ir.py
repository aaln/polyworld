...
def temporal_facts(game_version=None):
    ...
    if game_version in ("2026.9.15.1", "2026.9.15.2", "2026.9.15.3", "2026.9.16.2", "2026.9.16.3", "2026.9.16.5"):
        facts.update(
    ...
    if game_version in ("2026.9.15.3", "2026.9.16.2", "2026.9.16.3", "2026.9.16.5"):
    ...
    if game_version in ("2026.9.16.2", "2026.9.16.3", "2026.9.16.5"):
        facts.update(queries="Self data and visible object fields are fixed for one VM decision. Inventory and ability queries read live state; spell queries can include casts appended during the decision.", terrain="terrainWalkable includes team-known living building footprints. Destroyed buildings release occupied tiles after observation; attack range is measured from building footprint edges.", waves="Each surviving barracks spawns three creeps, six per lane every480ticks at24Hz. Barracks are objectKind5 and become exposed after lane towers fall.", balance="TowerHP950/1300/1950; DKheal36; Warlockmana30; Crossbow46+9/lvl; Berserker38+8/lvl; LichIceSpear48/range400000. Values verified at5c701f9; barracksHP950 in source, despite docs still saying900.")
    if game_version == "2026.9.16.5":
        facts.update(objective="Clearing any lane exposes two god guard towers, IDs28/29 forred and30/31 forblue. Both enemyguards mustfall before the god isexposed. Guards use gate-tower HP/range/damage. Barracks are optional for heroes, but creeps prioritize lane barracks beforeguards.", attack_move="attackMove(x,y) is now available to BASIC and records the same action as human attack-move.")
    return facts
