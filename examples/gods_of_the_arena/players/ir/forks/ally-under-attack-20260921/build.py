...
def main():
    ...
    new_name = 'richard_formation_combat_ally_under_attack_v1'
    new = replace(old, template=old.template.replace(token, ASSIST+token, 1),
                  parameters=old.parameters | {'assist_tiles': (8, 4, 12)},
                  memory=old.memory,
                  ...
                  'only; no opponent identity, seed or hidden state is read. Scratch variables mMin/mSum/mIndex/mKind/mId/mDx/mDy/mStep are reused because the parent is at the VM global-variable cap; each is reassigned before any later read. Focus intent does not guarantee '
    ...
    p = deepcopy(parent)
    p['id'] = 'gota_formation3600_ally_under_attack_v1'
    p['skill']['attack'] = {'operator': new_name, 'parameters': dict(parent['skill']['attack']['parameters']) | {'assist_tiles': 8}}
    ...
    p['update'].update(revision=p['update']['revision']+1, parent=digest(parent),
        change={'origin': 'Devin cycle 04 fork for relh v159 trigger', 'hypothesis': 'H-ALLY-UNDER-ATTACK-01',
    ...
    cm['interpretation'] = 'Parent frozen compiler modules; contracts add richard_formation_combat_ally_under_attack_v1 (template-level, no BASIC patch).'
