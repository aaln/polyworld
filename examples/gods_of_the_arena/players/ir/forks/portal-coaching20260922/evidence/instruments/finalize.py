...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--name',default='portal-coaching20260922');a=parser.parse_args()
    ...
    hosted_path=STUDY/'hosted/result.json'
    ...
    copy(STUDY/'hosted/plan.json',out/'evidence/hosted/plan.json')
    ...
    status=('passed' if hosted['passed'] else 'failed') if hosted else 'pending: prepared160-game fresh comparison awaits budget'
