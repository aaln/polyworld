"""Fork formation-adaptive-20260920 with H-ESCORT-ATKMOVE-01; never patch generated BASIC.
...
NEW_REL = 'moveAccepted = attackMove(moveX, moveY)\n'
...
def main():
    ...
    lane_name = 'formation_profile_route_escort_attackmove_v1'
    lane = replace(obs, template=obs.template.replace(OLD_REL, NEW_REL), actions=tuple(sorted(set(obs.actions)|{'attackMove'})),
                   parameters=obs.parameters,
                   meaning=obs.meaning + ' H-ESCORT-ATKMOVE-01: the escort/route destination is issued with attackMove instead of walkTo so a hero following its wave auto-acquires enemies within melee attack-move range (8 tiles) or its ranged attack range; walkTo suppresses acquisition entirely (acquireRadius = 0 while moving). Same target-clearing semantics as walkTo. Hypothesis, not a validated gain: falsified if red still loses seeds 54/101/202 vs Richard v135 with no basic-hit increase, or blue/red regress vs the parent.')
    ...
    p['update'].update(revision=p['update']['revision']+1, parent=digest(parent),
        change={'origin': 'Devin cycle 11 fork for Richard/relh trigger (farm-rate mechanism)', 'hypothesis': 'H-ESCORT-ATKMOVE-01 (route escort walkTo -> attackMove, zero new globals)',
    ...
    out.rmdir()
    (HERE/'policy.py').write_text('"""H-ESCORT-ATKMOVE-01 fork of the adaptive formation3600 IR; untested until README says otherwise."""\nPOLICY = '+pprint.pformat(p,width=110,sort_dicts=False)+'\n')
    ...
    cm['interpretation'] = 'Parent frozen compiler modules; contracts add formation_profile_route_escort_attackmove_v1 (template-level, no BASIC patch).'
