...
def build(stage):
    ...
    assert stage=='trial' and plan['games']==320 and len(plan['arms'])==8 and all(a['games']==40 for a in plan['arms'])
    ...
    for entry in candidates:
        ...
        exposure={n:sum(c['by_class'].get('3',{}).get('n',0) for c in cells if c['name']==n and c['ordinal']==3) for n in ['baseline',entry['name']]}
        entry['druid_exposure']=exposure
        lead=next(c for c in after if c['cell']=='blue-lead')
        gap=next(o for o in lead['opponents'] if o['label']=='khors:v114')['own_minus_rival']
        entry['blue_lead_mean_khors_gap']=gap
        entry['aggregate_gate_passed']=entry['score_gate_passed']
        entry['score_gate_passed']=entry['score_gate_passed'] and entry['late_score_gain_percent']>0 and gap>0 and min(exposure.values())>=10
