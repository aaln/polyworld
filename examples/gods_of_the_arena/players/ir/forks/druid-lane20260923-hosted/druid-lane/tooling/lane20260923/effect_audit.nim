...
import ../[game, sim, replays, content, maps]
...
while run.world.tick < run.replayData.hashes.len:
  ...
  if run.world.phase != Drafting:
    for i,h in run.world.heroes:
      if h.state == Dying: totals[i].add("dead_ticks",1)
      if h.hp > 0 and h.state != Dying:
        totals[i].add("alive_ticks",1)
        if h.canShop(): totals[i].add("keep_ticks",1)
        else:
          totals[i].add("field_ticks",1)
          if h.hp * 100 < h.maxHp * 30: totals[i].add("critical_field_ticks",1)
          var usefulShop=false
          if NoItem in h.inventory:
            for wanted in [CrimsonDagger,KnightArmor,BattleAxe,RuneCrossbow]:
              if wanted notin h.inventory and h.gold>=wanted.itemSpec.cost:usefulShop=true
          let baseDx=h.moveTileX-mapCoordinate(h.spawnPosition.x,h.team).int
          let baseDy=h.moveTileY-mapCoordinate(h.spawnPosition.z,h.team).int
          let headingHome=h.hasMoveTarget and not h.attackMoving and baseDx*baseDx+baseDy*baseDy<=9
          if headingHome:
            totals[i].add("base_path_ticks",1)
            if usefulShop:totals[i].add("base_path_affordable_core_item_ticks",1)
          if h.hp*100>=h.maxHp*60 and h.mana*100>=h.maxMana*20:
            totals[i].add("lane_ready_field_ticks",1)
            if headingHome:
              totals[i].add("lane_ready_homeward_ticks",1)
              if not usefulShop:totals[i].add("lane_ready_homeward_no_affordable_core_item_ticks",1)

          if h.hp * 100 >= h.maxHp * 75 and h.mana * 100 >= h.maxMana * 20:
            totals[i].add("operational_field_ticks",1)
            let dx=h.moveTileX-mapCoordinate(h.spawnPosition.x,h.team).int
            let dy=h.moveTileY-mapCoordinate(h.spawnPosition.z,h.team).int
            if h.hasMoveTarget and not h.attackMoving and dx*dx+dy*dy<=9:
              totals[i].add("operational_homeward_walk_ticks",1)
              var core=0
              for item in h.inventory:
                if item in {CrimsonDagger,KnightArmor,BattleAxe,RuneCrossbow}:inc core
              if core==4 or h.gold<500:totals[i].add("operational_homeward_no_shop_need_ticks",1)
  ...
  for e in run.world.events:
    ...
    if e.kind == Healing and target in 0..9:
      totals[target].add("healing_" & $e.cause,e.amount)
      if e.cause==AbilityEffect and actor==target:
        totals[target].add("self_healing",e.amount)
        if not run.world.heroes[target].canShop:totals[target].add("field_self_healing",e.amount)
      if e.cause == ItemEffect:
        totals[target].add("healing_item_" & $e.detail,e.amount)
    if e.kind == RecoveryInterrupted and target in 0..9:
      totals[target].add("recovery_interrupted_" & $e.detail,1)
    if e.kind == ItemPurchased and actor in 0..9:
      totals[actor].add("purchased_item_" & $e.detail,1)
    if e.kind == Damage:
      if actor in 0..9:
    ...
    if actor in 0..9:
      if e.kind == SpellReleased:
        spellCounts[actor].add($Ability(e.detail),1)
        if Ability(e.detail).abilitySpec.kind==Heal:totals[actor].add("accepted_healing_casts",1)
      if e.kind == GoldSpent:
        totals[actor].add("gold_spent",-e.amount)
        if e.cause == Buyback:
          totals[actor].add("accepted_buybacks",1)
          totals[actor].add("buyback_gold",-e.amount)
