...
def main():
    ...
    for label,count in [('baseline-practice',92),('lane-recovery-practice',92),('lane-recovery-openings',100),('lane-recovery-buyback',180),('lane-recovery-portals',84)]:
        d=read(STUDY/(label+'.json'));assert len(d['rows'])==count and all(r['passed'] for r in d['rows'])
    broad=read(STUDY/'lane-recovery-scenarios.json');assert broad['passed'] and broad['checks']==126
    ...
    summary={'passed':True,'source_sha256':sha(pair/'policy.bas'),'lane_recovery_cases':92,'opening_cases':100,'inherited_buyback_cases':180,'portal_cases':84,'broader_cases':126,'full_native_games':16,'max_fixture_instructions':max(read(STUDY/('lane-recovery-'+n+'.json'))['max_instructions'] for n in ['practice','openings','buyback','portals','scenarios']),'max_fixture_work':max(read(STUDY/('lane-recovery-'+n+'.json'))['max_work'] for n in ['practice','openings','buyback','portals','scenarios']),'competitive_inference':False}
