' Generated from gota_score_opportunities20260922; gota-semantic-policy/1; gota-bassy/score-opportunities-2026-09-22-r1
...
if active = 1 then
  ...
  farmPickId = 0
  farmPickScore = -1000000
  structurePickId = 0
  structurePickScore = -1000000
  bestId = 0
  bestScore = -1000000
  ...
  structurePickId = id
  structurePickScore = score
  structurePickKind = kind
  structurePickHp = hp
  structurePickX = x
  structurePickY = y
  structurePickDistance = distance
              end if
            end if
          end if
        end if
...
if active = 1 then
  ...
  if structurePickId > 0 and structurePickScore > bestScore then
    bestId = structurePickId
    bestScore = structurePickScore
    bestKind = structurePickKind
    bestHp = structurePickHp
    bestX = structurePickX
    bestY = structurePickY
    bestDistance = structurePickDistance
  end if
  if farmPickId > 0 and farmPickScore > bestScore then
    bestId = farmPickId
    bestScore = farmPickScore
    bestKind = farmPickKind
    bestHp = farmPickHp
    bestX = farmPickX
    bestY = farmPickY
    bestDistance = farmPickDistance
  end if
  if heroPickId > 0 and heroPickScore > bestScore then
    bestId = heroPickId
    bestScore = heroPickScore
    bestKind = heroPickKind
    bestHp = heroPickHp
    bestX = heroPickX
    bestY = heroPickY
    bestDistance = heroPickDistance
  end if
end if
