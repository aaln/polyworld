"""Reflect the complete hosted result into the exact tested lane recovery pair."""
import json
import pprint
import shutil
import subprocess
...
OUT = ROOT / 'examples/gods_of_the_arena/players/ir/forks/lane-recovery20260923-hosted'


def main():
    report = read(STUDY / 'trial/report.json')
    assert report['complete'] and report['games'] == 320
    assert len(read(STUDY / 'trial/effects-summary.json')['rows']) == 32
    assert not OUT.exists(), 'Preserve sealed result capsules'
    prior = local.OUT / 'lane-recovery'
    ...
    shutil.copy2(STUDY / 'diagnostic-metric-addendum.json', evidence / 'diagnostic-metric-addendum.json')
    write(evidence / 'effect-audit-provenance.json', {
        'source_sha256': sha(ROOT / 'games/gods_of_the_arena/instruments/lane20260923/effect_audit.nim'),
        'binary_sha256': sha(STUDY / 'sustain-effect-audit'),
        'build_log_sha256': sha(STUDY / 'build-effect-audit.log'),
        'original_source_sha256': sha(STUDY / 'effect-audit-original.nim'),
        'original_binary_sha256': sha(STUDY / 'effect-audit-original')})
    ...
    p['belief']['claims']['CompetitiveGain'].update(
        ...
        claim=f"The frozen 320-game qualification rule was met: {report['deployment_qualified']}. "
              ...
              'This decision applies to the frozen four-context roster and complete candidate; '
