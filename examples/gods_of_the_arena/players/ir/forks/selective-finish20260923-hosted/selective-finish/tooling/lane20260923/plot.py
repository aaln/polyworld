...
import numpy as np
ROOT=Path(__file__).resolve().parents[4];STUDY=ROOT.parent/'polyworld/tmp/gota-lane-recovery-20260923'
...
ax.set_xticks([0,1,2,3],['Red lead','Blue lead','Red late','Blue late']);ax.set_ylabel('Mean individual points');ax.set_title('Score by context · 40 games per bar');ax.legend(fontsize=8);ax.spines[['top','right']].set_visible(False)
...
fig.suptitle('GOTA lane recovery · '+a.stage+' · 2026.9.22.3',fontsize=15)
fig.text(.06,.02,'Left: 95% whole-game bootstrap intervals. Right: exact mean score decomposition, including zero-clamp/rounding residual.\nFixed mixed rosters; later draft includes two fixed reference teammates. Effects are descriptive. One preselected candidate; 320 fresh held-out games.',fontsize=9,color='#52606d')
