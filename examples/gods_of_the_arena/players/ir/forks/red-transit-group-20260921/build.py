"""Fork formation-adaptive-20260920 with H-RED-TRANSIT-01; never patch generated BASIC.

Uses the parent's frozen compiler.zip + contracts.json. Replaces the red-only caster transit
state contract with one that only activates the transit when the previous tick's weighted
nearby-ally count (mAllies: hero within 6 tiles = 3, creep = 1; computed by the attack scan
later in the same tick, persists in the VM globals) is >= transit_min_allies. Zero new globals.
Variant is selected by the directory name: red-transit-group (6 = at least two allied heroes)
or red-transit-off (99 = transit never activates).
"""
...
VARIANT = 'group' if 'group' in HERE.name else 'off'
MIN_ALLIES = {'group': 6, 'off': 99}[VARIANT]
...
def main():
    ...
    p['update'].update(revision=p['update']['revision']+1, parent=digest(parent),
        change={'origin': 'Devin cycle 07 fork for Richard/relh red trigger', 'hypothesis': 'H-RED-TRANSIT-01 ' + VARIANT + ' (transit gated on previous-tick mAllies >= %d, zero new globals)' % MIN_ALLIES,
                'layer': 'skill/strategy (transit_state) + belief', 'rule': 'R_profile_transit'},
    ...
    out.rmdir()
    (HERE/'policy.py').write_text('"""H-RED-TRANSIT-01 fork of the adaptive formation3600 IR; untested until README says otherwise."""\nPOLICY = '+pprint.pformat(p,width=110,sort_dicts=False)+'\n')
    ...
    cm['interpretation'] = 'Parent frozen compiler modules; contracts add formation_profile_transit_state_gated_v1 (template-level, no BASIC patch).'
