' Generated from gota_balance20260922_arcanist; gota-semantic-policy/1; gota-bassy/balance-2026-09-22-r1
  ...
  if selfChannelTicks = 0 and selfStunTicks = 0 and selfRootTicks = 0 then
    if resumeTarget > 0 then
      ...
      if retreat = 0 and selfTarget > 0 and selfAttacksLanded > previousHits then
        ...
        walkTo(originX + facingSign * (myX + 0.25), originY + facingSign * (myY + 0.25))
        active = 0
      end if
    end if
  end if
  ...
  while i < 96 and i < objects
    ...
    if idx < objects then
      ...
      if hp > 0 then
        ...
        kind = objectKind(idx)
        team = objectTeam(idx)
        x = (originX + facingSign * objectX(idx))
        y = (originY + facingSign * objectY(idx))
        dx = x - myX
        dy = y - myY
    ...
    if ordinal <= 1 and homeEnemyHeroes > 0 and homeDefenders < 2 then
      ...
      if dx * dx + dy * dy > 625 and selfHp * 100 >= selfMaxHp * 60 then
        if selfPortalCooldown = 0 and threatDistance > 225 and worldTick - hurtTick > tickRate * 2 then
          if useItemAt(portalSlot, originX + facingSign * (homeX), originY + facingSign * (homeY)) = 1 then
            stopped = 1
          end if
        end if
      end if
...
' @rule R_recover
if active = 1 then
  if selfHp * 100 < selfMaxHp * 24 then
    retreat = 1
  end if
  if inOwnSpawn() = 1 then
    if selfHp * 10 >= selfMaxHp * 9 and selfMana * 10 >= selfMaxMana * 8 then
      ...
      if worldTick >= moveTick then
        walkTo(originX + facingSign * (spawnX), originY + facingSign * (spawnY))
  ...
  if stopped = 0 and retreat = 1 then
    ...
    if portalSlot >= 0 and selfPortalCooldown = 0 then
      if dx * dx + dy * dy > 400 and threatDistance > 144 then
        if useItemAt(portalSlot, originX + facingSign * (spawnX), originY + facingSign * (spawnY)) = 1 then
          stopped = 1
        end if
      end if
    end if
...
if active = 1 then
  if stopped = 0 then
    ...
    if unsafe = 1 and selfRootTicks = 0 then
      ...
      if worldTick >= moveTick then
        walkTo(originX + facingSign * (myX + stepX), originY + facingSign * (myY + stepY))
        moveTick = worldTick + 24
      end if
      stopped = 1
    end if
  ...
  if stopped = 0 and selfClass = Crossbowman and bestKind = 3 and bestId > 0 then
    if bestDistance > 25 and bestDistance <= 64 and enemyPower <= friendPower then
      if selfRootTicks = 0 then
        walkTo(originX + facingSign * ((myX + bestX) / 2), originY + facingSign * ((myY + bestY) / 2))
...
if active = 1 then
  if stopped = 0 then
    ...
    if canShop() = 1 and portalSlot >= 0 and selfPortalCooldown = 0 then
      if retreat = 0 and threatDistance > 144 and forwardScore < 1000000 then
        ...
        if dx * dx + dy * dy > 400 then
          if useItemAt(portalSlot, originX + facingSign * (forwardX), originY + facingSign * (forwardY)) = 1 then
            stopped = 1
          end if
        end if
      end if
    ...
    if stopped = 0 and selfRootTicks = 0 and worldTick >= moveTick then
      attackMove(originX + facingSign * (goalX), originY + facingSign * (goalY))
      moveTick = worldTick + 24
    end if
  end if
end if
