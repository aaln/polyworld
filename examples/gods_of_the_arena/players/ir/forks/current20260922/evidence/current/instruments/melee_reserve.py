...
PARENT=ROOT/'tmp/gota-targets-20260922/candidates/practiced'
VERSION='gota-bassy/microplay-2026-09-22-r4'
...
''',{},(),(),('walkTo','attackTarget'),
 'After an observed landed hit, walk for one physics tick to clear recovery, then reacquire the remembered public target. Same-tick walk and attack does not reset the new engine swing. Never cancel unfinished windup or a portal channel; clear pending reacquisition after death.',('previousHits','resumeTarget'))
...
''',{},(),(),('walkTo',),
 'For safe Crossbowman creep farming, step inside the six-tile XP radius instead of stopping at its 6.5-tile attack range. Override stale navigation throttles before an out-of-range XP last hit; defer while tower safety or recovery owns movement.')

HOME_PORTAL=contract(r'''
if stopped = 0 and retreat = 0 and portalSlot >= 0 and selfRootTicks = 0 then
...
def economy(potions):
    ...
    for item,cost in order:
        ...
        if item in (1,21):condition += ' and (has16 = 1 or selfAttackRange >= 180000)'
        lines += [f'  if {condition} and budget >= {cost} and empty > 0 then',f'    if buyItem({item}) = 1 then',f'      budget = budget - {cost}','      empty = empty - 1']
        if item not in (1,21):lines += ['      gearCount = gearCount + 1',f'      has{item} = 1']
        lines += ['    end if','  end if']
    if potions:lines += ['  if (has16 = 1 or selfAttackRange >= 180000) and healCount > 0 and healCount < 2 and budget >= 30 then','    buyItem(1)','  end if']
    ...
    return contract('\n'.join(lines),{'shop_gold':(500,300,1200)},(),(),('buyItem','useItem'),
      ('Reserve potion and portal slots; buy dagger, armor, axe and crossbow instead of low-impact boots.' if potions else 'Reserve one portal slot; spend on five permanent items: dagger, armor, axe, crossbow and longsword.')+' For short-range heroes, reserve gold for missing armor before purchasing or replenishing scrolls/potions; mark accepted gear immediately. Safely return to shop when useful equipment remains missing and gold is banked.',('restock',))
...
def build(name,mode):
    ...
    p['update']={'revision':3,'parent':parent,'change':{'origin':'Broad armor reserve harmed native red performance; restrict the reserve to short-range heroes while preserving ranged mobility and test against practiced parent','variant':mode},'needs_review':['belief/Microplay'],'evidence':[{'artifact':'games/gods_of_the_arena/experiments/2026-09-21-targets-microplay.md'}]}
