' Generated from gota_targets20260922_reserve; gota-semantic-policy/1; gota-bassy/microplay-2026-09-22-r3
...
if active = 1 then
  ...
  if canShop() = 1 then
    ...
    if portalCount = 0 and has16 = 1 and budget >= 100 and empty > 0 then
    ...
    if healCount = 0 and has16 = 1 and budget >= 30 and empty > 0 then
      if buyItem(1) = 1 then
        budget = budget - 30
        empty = empty - 1
      end if
    end if
    if has16 = 1 and healCount > 0 and healCount < 2 and budget >= 30 then
      buyItem(1)
    end if
  else
