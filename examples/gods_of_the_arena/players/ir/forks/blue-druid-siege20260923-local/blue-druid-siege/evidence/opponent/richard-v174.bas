' Gods of the Arena GameVersion 58 Ranger/Berserker draft correction.
' Favor the newly strengthened Crossbowman and Warlock, avoid the observed
' low-scoring Druid Warden and Death Knight, and retain the v32 battle actor.
sub chooseHero()
  ...
  while candidateClass < 10
    if heroAvailable(candidateClass) then
      ...
      if candidateClass = 6 then
        draftScore = draftScore + 25
      end if
      ' Break the Berserker faction tie only when Ranger and Berserker compete.
      if candidateClass = 1 and heroAvailable(9) then
        draftScore = draftScore + 2
      end if
      if candidateClass = 8 then
        draftScore = draftScore + 10
      end if
      if candidateClass = 3 or candidateClass = 5 then
        draftScore = draftScore - 20
      end if
