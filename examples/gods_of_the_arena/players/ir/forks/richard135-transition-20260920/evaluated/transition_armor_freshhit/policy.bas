' Generated from gota_richard_transition_armor_freshhit; gota-semantic-policy/1; gota-basic/1
...
' @rule R1
gaActive = 0
if selfTeam = 0 and worldTick >= 4800 then
  gaActive = 1
  defActive = 0
  bestId = 0
  ...
  if defGroupSize = 5 then
    ...
    if objectTeam(defI) = selfTeam and objectHp(defI) > 0 then
        perimeterObjX = objectX(defI)
        perimeterObjY = objectY(defI)
  ...
  defI = 0
  while defI < macroIndex and defI < 128
    defKind = objectKind(defI)
    coreTarget = objectTeam(defI)
    pairAnchorHp = objectHp(defI)
    if pairAnchorHp > 0 and objectAlive(defI) then
      ...
      if coreTarget = selfTeam then
        ...
        if defKind = 2 or defKind = 3 then
          if (perimeterObjX - 105) * (perimeterObjX - 105) + (perimeterObjY - 11) * (perimeterObjY - 11) <= 1296 then
            backdoorNearer = backdoorNearer + 1
          end if
        end if
        if defKind = 2 then
          coreScore = defD * 4 + pairAnchorHp
          if objectClass(defI) = 1 and (perimeterObjX - 105) * (perimeterObjX - 105) + (perimeterObjY - 11) * (perimeterObjY - 11) <= 1600 then
            coreScore = coreScore - 100000
          end if
          if defD <= 484 and coreScore < defScore then
            defFront = objectId(defI)
            defScore = coreScore
            backdoorGuardA = perimeterObjX
            backdoorGuardB = perimeterObjY
          end if
  ...
  if worldTick <= backdoorLastSeen or worldTick > backdoorLastSeen + 1 then
    ...
    perimeterAnchor = coreValue
  end if
  if defCount > 0 then
    perimeterLastCombat = worldTick
  end if
  if coreRespond >= 2 or coreValue < perimeterAnchor then
    defUntil = worldTick + 360
    defThreatX = defFrontX
    defThreatY = defFrontY
    criticalUntil = 0
  else
    if macroIndex <= 128 and backdoorNearer = 0 then
      if criticalUntil = 0 then
        criticalUntil = worldTick
      end if
      if worldTick - criticalUntil >= 72 then
        defUntil = 0
      end if
    else
      criticalUntil = 0
  ...
  if defFront <> 0 and defScore < 0 then
    bestId = defFront
    bestDistance = defScore + 100000
    backdoorActive = 1
  end if
  defDx = selfX - defPointX
  defDy = selfY - defPointY
  gaTethered = 0
  if defDx * defDx + defDy * defDy > 20 * 20 and (defSentry = 0 or gaReady) and backdoorActive = 0 then
    bestId = 0
    defGoalX = defPointX
    defGoalY = defPointY
    gaTethered = 1
  end if
  if defSentry = 0 and defCount = 0 and backdoorNearer = 0 and macroIndex <= 128 then
    if worldTick - perimeterLastCombat >= 240 then
      if selfClass = 6 and selfHp * 5 >= selfMaxHp * 3 then
        if (defPointX - 105) * (defPointX - 105) + (defPointY - 11) * (defPointY - 11) <= 3600 then
          defGoalX = defPointX - 10
          defGoalY = defPointY + 10
          bestId = 0
          gaTethered = 1
          perimeterEnabled = 1
        end if
...
' @rule R2
if gaActive and backdoorActive and bestId <> 0 then
  motionActive = 0
  motionUntil = 0
  mSpell = 1
  if selfClass = 6 then
  ...
  if worldTick = motionLastTick + 1 and selfAttacksLanded > motionLastHits and selfClass <> 9 then
    mDirX = 0
    mDirY = 0
