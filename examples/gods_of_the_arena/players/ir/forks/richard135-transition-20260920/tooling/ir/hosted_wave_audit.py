...
from vm_log_status import capture_failed


def verify_vm_validity(folder):
    """Require all ten VMs, using the game's equivalent structured status if logs were lost.

    Published 2026.9.16.2 (and the earlier .3 build) sets each status exit_code
    from the same sticky BASIC failure flag reported by the headless VM summary.
    A completed API episode alone is
    ...
    proof = {}
    if log_path.exists():
        proof['log_sha256'] = digest(log_path.read_bytes())
    elif (folder / 'game-log-response.json').exists():
        receipt = read(folder / 'game-log-response.json')
        episode_id = read(folder / 'episode.json')['id']
        missing_receipt = (receipt.get('episode_id') == episode_id and
            receipt.get('status_code') == 404 and
            receipt.get('path') == f'/v2/episode-requests/{episode_id}/artifacts/logs' and
            str(receipt.get('body', {}).get('detail', '')).startswith('No logs found for job '))
        if missing_receipt:
            proof['log_response_sha256'] = digest((folder / 'game-log-response.json').read_bytes())
    active = re.findall(r'scripts: (\d+)/10 active', log)
    if active:
        if active[-1] != '10':
            raise ValueError('A BASIC VM was disabled')
        proof['method'] = 'headless_summary'
    elif (capture_failed(log) or missing_receipt) and (folder / 'player-status.json').exists():
        ...
        proof.update(method=('structured_player_status_after_official_log404' if missing_receipt else 'structured_player_status_after_log_capture_failure'),
