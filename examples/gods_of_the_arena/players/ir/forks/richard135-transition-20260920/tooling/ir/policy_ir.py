...
def grounded(policy):
    ...
    return {
        "situation": {
            "observation": "Current host snapshots and visibility-filtered objects; integer tiles; six inventory slots.",
        ...
        "belief": {
            ...
            "uncertainty": ("Unseen enemies are unobserved. Visible object target IDs are exposed; zero can mean no target or a target outside visibility. Future enemy intentions remain unknown." if policy["execution"]["game_version"] in ("2026.9.15.3", "2026.9.16.2", "2026.9.16.3", "2026.9.16.5") else "Unseen enemies are unobserved. Tower targets and enemy intentions are not exposed."),
        },
...
def validate(policy):
    ...
    for name, skill in policy["skill"].items():
        ...
        if skill["operator"] in ("confirmed_hit_kite", "targeted_hit_kite", "spell_hit_kite", "motion_feedback", "cadence_motion", "tower_detour", "opportunity_enemy") and policy["execution"]["game_version"] not in ("2026.9.15.3", "2026.9.16.2", "2026.9.16.3", "2026.9.16.5"):
            raise ValueError("confirmed_hit_kite requires the published 2026.9.15.3 combat observations")
        if skill["operator"] in ("building_enemy", "class_cadence", "class_building", "class_building_two", "bounded_pursuit", "macro_objectives", "macro_lanes", "macro_cadence", "rush_objective", "rush_route", "coached_lane", "coached_route", "coached_blockers", "coached_ranged", "coached_focus") and policy["execution"]["game_version"] not in ("2026.9.16.2", "2026.9.16.3", "2026.9.16.5"):
            raise ValueError("building_enemy requires verified 2026.9.16.2 building mechanics")
        if skill["operator"] in ("coached_guards", "coached_convoy", "coached_defense", "coached_recovery", "coached_strike", "lich_lane_route", "lich_guard_observe", "lich_convoy_observe", "lich_push_route", "coached_finish", "rush_defense", "rush_defense_v2", "rush_defense_v3", "rush_defense_v4", "defense_cadence", "defensive_recovery", "opening_guard", "rush_sentries", "core_wave_defense", "spread_defense", "released_defense", "reachable_core_defense", "released_reachable_defense", "released_sentry_defense", "fused_core_defense", "solo_base_defense", "assigned_solo_defense", "protected_solo_defense", "protected_assigned_defense", "protected_near_defense", "isolated_solo_defense", "isolated_assigned_defense", "isolated_near_defense", "blue_isolated_assigned", "blue_warning_defense", "objective_perimeter", "forward_perimeter", "lineup_perimeter_observe", "lineup_perimeter_route", "lineup_arrival_observe", "lineup_pressure_observe", "lineup_coverage_observe", "lineup_breach_observe", "lineup_core_pressure_observe", "lineup_core_breach_observe", "lineup_farm_core_observe", "lineup_bounded_core_observe", "lineup_bounded_farm_observe", "mage_reserve", "red_warlock_cadence", "lineup_rolling_creep_observe", "lineup_counterpush_observe", "lineup_counterpush_observe_v2", "lineup_counterpush_route", "lineup_victory_observe", "lineup_counterwatch_observe", "lineup_shared_engagement", "lineup_ally_assist", "lineup_caster_assist", "lineup_caster_assist_v2", "lineup_supported_defense", "lineup_reachable_support", "lineup_anchored_support", "lineup_anchored_readiness", "lineup_post_defense", "lineup_post_arrival", "lineup_middle_rush", "lineup_live_anchor", "lineup_live_anchor_core", "lineup_paired_siege", "lineup_paired_legacy", "lineup_paired_scoped", "lineup_watch_defense", "lineup_quiet_watch", "lineup_protected_watch", "lineup_coached_split", "lineup_coached_center", "lineup_coached_assembled", "lineup_coached_mobile", "defend_or_wave") and policy["execution"]["game_version"] != "2026.9.16.5":
            raise ValueError("God guards require the verified 2026.9.16.5 release")
        CONTRACTS[skill["operator"]].source(skill["parameters"])
        if skill['operator'] in ('motion_feedback', 'cadence_motion', 'class_cadence') and skill['parameters']['min_ticks'] > skill['parameters']['max_ticks']:
            raise ValueError('minimum retreat duration exceeds its maximum')
        if skill["operator"] in ("buy_ordered_loadout", "selective_loadout"):
            items = [value for name, value in skill["parameters"].items() if name.endswith('_item')]
