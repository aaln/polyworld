...
from autoresearch_inward_fused import STUDY as S,RAW
...
class FusedTests(unittest.TestCase):
    ...
    def test_no_enemy_dense236_budget_equipment_all_classes_both_colors(self):
        rows=[]
        for team in (0,1):
            for slot in range(5):
                c=scene(team,slot,dense=240)
    ...
    def test_dense_mixed_public_enemies_and_allies(self):
        ...
        self.assertLessEqual(max(r['instructions'] for r in rows),20000)
        self.assertLessEqual(max(r['work'] for r in rows),50000)
        write(S/'mixed-budget.json',dict(decisions=len(rows),max_instructions=max(r['instructions'] for r in rows),max_work=max(r['work'] for r in rows),scope='236 visible objects including40 enemy creeps, two colors/allclasses, no alarm.'))
