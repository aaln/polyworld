...
from coordinated_readiness import make, VARIANTS, STUDY
from policy_ir import compile_policy, extract, write


class CoordinatedReadinessTests(unittest.TestCase):
    ...
    def test_nearby_but_distant_from_enemy_does_not_count(self):
        for name in VARIANTS[2:]:
            self.assertEqual(self.play(name,self.states(healthy=True),('bestId','dhHeld'))[-1]['memory'],{'bestId':0,'dhHeld':1})

    def test_reachable_or_committed_support_and_emergency_allow_response(self):
        for mode in ('reachable','committed','tower'):
            cases=self.states(healthy=mode!='tower',tower=mode=='tower')
            for c in cases:
                if mode=='reachable':c['objects'][5].update(objectX=94,objectY=15)
                if mode=='committed':c['objects'][5]['objectTarget']=106
            for name in VARIANTS[2:]:
    ...
    def test_hold_then_engage_together_when_enemy_enters_close_defense(self):
        for name in VARIANTS[2:]:
            for slot in (0,2):
                cases=self.states(slot)
                # The leading enemy is outside caster eligibility and too far
                # from healthy support; no teammate has committed yet.
                before=self.play(name,cases,('bestId',))[-1]['memory']['bestId']
                self.assertEqual(before,0)
                from copy import deepcopy
                closer=deepcopy(cases[-1]);closer['self']['worldTick']=102
                for o in closer['objects']:
                    if o['objectKind']==2 and o['objectTeam']==1:
                        o.update(objectX=98,objectY=14)
                after=self.play(name,[*cases,closer],('bestId',))[-1]['memory']['bestId']
                self.assertEqual(after,106)
