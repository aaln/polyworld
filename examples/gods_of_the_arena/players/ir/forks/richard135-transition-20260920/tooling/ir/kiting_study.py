...
def main(definition=None):
    definition=definition or {"hypothesis":"burst_kite","screen_seed":719000,"validation_seed":720000}
    ...
    if not baseline.exists():baseline.write_bytes((ROOT/"tmp/gota-ir/runtime-2026.9.15.2/examples/gods_of_the_arena/players/base.bas").read_bytes())
    ...
    if not (d/"plan.json").exists():
        ...
        plan={"created_at":datetime.now(timezone.utc).isoformat(),"game_version":"2026.9.15.2",
              "hypothesis":definition["hypothesis"],
              "runtime":read(d/"runtime.json"),"runtime_sha256":digest((d/"runtime.json").read_bytes()),
              "published_source":"a6112c5d92c1efc86b7b30235c408106bc497242",
              ...
              "instruments":{str(p.relative_to(ROOT)):digest(p.read_bytes()) for p in
                             [HERE/n for n in ["kiting_study.py","episode_kiting.nim","audit_current.nim","binding.py","policy_ir.py","independent_ir.py","hypothesis_study.py",*definition.get("instruments",[])]]},
              "screen":cases(definition["screen_seed"],40),"validation":cases(definition["validation_seed"],120),
              "screen_rule":"40 fresh balanced independent cases, candidate and v2 and default each vs nine defaults. Candidate wins >= each reference; deaths <=85% of v2, hits >=80%; movement in >=30% of cases, >=90% normal retreats after a replay-observed hit within18ticks; no corrected adverse class/regression flag. Directional screen only.",
    ...
    for stage in [args.stage]:
        ...
        activated=len(active)>=len(hs)*.3 and verified>0 and verified/max(1,verified+unverified)>=.9
