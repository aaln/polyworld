...
from g002_watch import make, STUDY, VARIANTS
...
class WatchTests(unittest.TestCase):
    ...
    def test_attackers_outside_self_circle_but_near_objective(self):
        case=gate_case()
        self.assertEqual(self.play('deployed',[case])[0]['memory']['bestId'],0)
        for name in VARIANTS[1:]:
            row=self.play(name,[case],MEMORY)[0]
            self.assertIn(row['memory']['bestId'],range(105,110))
            self.assertTrue(any(a['command']=='attackTarget' for a in row['actions']))

    def test_quiet_advance_keeps_single_survivor_recall(self):
        for name,wait in [('watch20',480),('watch30',720)]:
            ...
            self.assertEqual(r[3]['memory']['bestId'],105)
