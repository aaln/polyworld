...
def main():
    ...
    path=HERE/'kiting-20260915-screen.json';write(path,result)
    ...
    updated['belief']['claims']['B_kite_timing']={'claim':f"Replay-observed basic hit within18ticks before {act['verified_bursts']}/{act['verified_bursts']+act['unverified_bursts']} normal retreats; displacement observed in {act['games']}/40 games. Tile timing remains an imperfect inference. All120 complete action/state replays verified.",'status':'supported' if fraction>=.9 and act['verified_bursts']>0 else 'requires_review','evidence':[reference]}
    ...
    next(r for r in updated['strategy'] if r['id']=='R2')['for'].append('G_glory')
    ...
    write(HERE/'hypotheses/burst_kite.evaluated.ir.json',updated)
    (HERE/'hypotheses/burst_kite.evaluated.bas').write_text(source)
