...
from coach_assembled import make, STUDY, VARIANTS
...
class AssembledTests(unittest.TestCase):
    ...
    def sequence(self, slot):
        cases = [gate_case(100), gate_case(101,0), gate_case(460,0), gate_case(461)]
        for c in cases:
            c['self']['selfClass'] = class_id(0,slot)
            c['objects'] += [f.obj(100+i,kind=2,team=0,x=100+i,y=15,hp=400) for i in range(5)]
        return cases
    ...
    def test_live_contact_delays_release_dead_enemy_does_not(self):
        for name in VARIANTS[1:]:
            for living in (True,False):
                cases=self.sequence(1)[:3]
                for case in cases[1:]:
                    e=f.obj(555,kind=2,team=1,x=104,y=10,hp=100 if living else 0)
    ...
    def test_core_damage_recalls_attackers(self):
        for name in VARIANTS[1:]:
            cases=self.sequence(1)[:3]
            hit=deepcopy(cases[-1]);hit['self']['worldTick']=461
    ...
    def test_group_travel_and_unassembled_allies_block_release(self):
        for name in VARIANTS[1:]:
            cases=self.sequence(1)[:3]
            for c in cases[1:]:
                c['objects'] += [f.obj(105+i,kind=2,team=1,x=70+i,y=50,hp=400) for i in range(5)]
            self.assertEqual(self.play(name,cases,MEMORY)[-1]['memory']['cpMode'],0)
            cases=self.sequence(1)[:3]
            for c in cases[1:]:c['objects']=[o for o in c['objects'] if o['objectId'] not in (102,103,104)]
            self.assertEqual(self.play(name,cases,MEMORY)[-1]['memory']['cpMode'],0)
