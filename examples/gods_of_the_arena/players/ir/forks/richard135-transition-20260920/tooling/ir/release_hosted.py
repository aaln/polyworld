...
def upload(directory):
    ...
    for name in screen['selected']:
        ...
        metadata = {'name': 'aaron-gota-ir-release-' + name.replace('_', '-') + '-0916',
        ...
        arm_plan.update(run_id='gota-release-20260916-' + name, policy_version=version['id'],
...
def summarize(parts):
    ...
    for row in rows:
        row['legacy_glory_100'] = row['glory']
        row['glory'] = row['win'] * (row['xp'] - 200 * row['ticks'] / 1440)
    count = len(rows)
    ...
    return {'glory_penalty_xp_per_minute': 200, 'glory_scope': 'Current local tournament diagnostic; league objective is binary fort win', 'games': count, 'wins': sum(r['win'] for r in rows),
...
def compare(directory, confirmation=False):
    ...
    selected = [plan['candidate']] if confirmation else read(directory / 'local/screen-result.json')['selected']
    names = ['v2', 'cadence', *selected]
    arms = {}
    seeds = set()
    if confirmation:
        discovery = read(directory / 'hosted-discovery/result.json')
        seeds = {r['seed'] for a in discovery['arms'].values() for r in a['rows']}
        for previous in plan.get('excluded_discovery_results', []):
            seeds.update(r['seed'] for a in read(previous)['arms'].values() for r in a['rows'])
        for previous in plan.get('excluded_rival_results', []):
            seeds.update(r['seed'] for a in read(previous)['rivals'].values() for r in a['rows'])
    for name in names:
    ...
    for name in selected:
        ...
        for control in ['v2', 'cadence']:
            ...
            comparisons[name][control] = {'gain': gain, 'p': p, 'adverse_classes': adverse,
                                          'passed': gain >= .05 and (not confirmation or (p < .025 and not adverse)) and (not plan.get('discovery_reject_adverse_classes', False) or not adverse)}
        guards = a['death_rate'] <= 1.1 * arms['cadence']['death_rate'] and a['xp'] >= .8 * arms['cadence']['xp'] and a['equipment_games'] == a['games']
    ...
    write(path, result)
    for name in selected:
        parent = directory / ('hosted-discovery' if confirmation else 'local/screen-feedback') / name
        if confirmation:
        ...
        claim = (f"Completed hosted {result['stage']} against fixed current top-nine incumbents on {plan['game_version']}: "
                 f"{a['wins']}/{a['games']} wins, v2 {arms['v2']['wins']}/{arms['v2']['games']}, cadence {arms['cadence']['wins']}/{arms['cadence']['games']}. "
        ...
        record(parent / 'policy.ir.json', directory / 'local/candidates' / name / 'policy.bas', claim,
...
def prepare_confirmation(directory):
    ...
    plan.update(candidate=name, discovery_sha256=digest((discovery / 'result.json').read_bytes()),
                ...
                excluded_discovery_results=old.get('excluded_discovery_results', []))
    excluded_rivals = list(old.get('excluded_rival_results', []))
    if (directory / 'rival-matchups/result.json').exists():
        excluded_rivals.append(str(directory / 'rival-matchups/result.json'))
    if excluded_rivals:
        plan['excluded_rival_results'] = list(dict.fromkeys(excluded_rivals))
    if (root / 'plan.json').exists() and read(root / 'plan.json') != plan:
        raise ValueError('Confirmation selection changed')
    write(root / 'plan.json', plan)
    for arm in ['v2', 'cadence', name]:
        prior = read(discovery / arm / 'plan.json')
        for index in range(4):
            folder = root / arm / f'part-{index}'
            folder.mkdir(parents=True, exist_ok=True)
            write(folder / 'plan.json', prior | {'run_id': f'gota-release-confirm-20260916-{study_tag}-{arm}-{index}',
