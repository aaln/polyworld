' Generated from gota_targets20260922_growth; gota-semantic-policy/1; gota-bassy/microplay-2026-09-22
...
if active = 1 then
  ...
  if canShop() = 1 then
    ...
    if has13 = 0 and budget >= 150 and empty > 0 then
      if buyItem(13) = 1 then
        budget = budget - 150
        empty = empty - 1
        gearCount = gearCount + 1
      end if
    ...
    if gearCount < 5 and selfGold >= 500 and threatDistance > 225 then
      restock = 1
    end if
  end if
  if restock = 1 then
