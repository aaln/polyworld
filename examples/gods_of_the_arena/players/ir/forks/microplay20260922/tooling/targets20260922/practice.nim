...
proc step(g: Game) =
  ...
  if trace:
    for i,vm in g.heroVms:
      if vm != nil:
        let h=g.world.heroes[i]
        stderr.writeLine($(%*{"tick":g.world.tick,"x":h.position.x,"z":h.position.z,"state": $h.state,"target":h.attackObjectId,"swing":h.swingTicks,"hits":h.attacksLanded,"active":vm.runtime.getGlobal("active"),"best":vm.runtime.getGlobal("bestId"),"distance":vm.runtime.getGlobal("bestDistance"),"stopped":vm.runtime.getGlobal("stopped"),"retreat":vm.runtime.getGlobal("retreat"),"moveTick":vm.runtime.getGlobal("moveTick")}))
  for vm in g.heroVms:
    if vm != nil:
...
for team in Team:
  for class in HeroClass:
    ...
    for i in 0..<360:
      ...
      trace=getEnv("PRACTICE_TRACE")=="1" and team.ord==0 and class==Ranger and i<60
      g.step()
      trace=false
  ...
  block:
    ...
    for z in 32..<96:
      for x in 32..<90:
        var open=true
        for k in 0..7:
          if not navigationOpen(GroundLayer,x+k,z):open=false
        if not open:continue
        let a=pathPoint(GroundLayer,x,z)
        let b=pathPoint(GroundLayer,x+6,z)
        origin=WorldPoint(x:a.x*Unit,y:a.y*Unit,z:a.z*Unit)
        aim=WorldPoint(x:b.x*Unit+WorldScale div 4,y:b.y*Unit,z:b.z*Unit)
        h.place(origin)
        if not h.canShop:found=true;break
      if found:break
    ...
    for i in 0..<180:
      trace=getEnv("PRACTICE_TRACE")=="1" and team.ord==0 and i<60
      h.hp=h.maxHp;h.mana=0
      for f in g.world.footmen.mitems:
        if f.hp>0:f.place(aim)
      g.step()
