...
for team in Team:
  let game = quiet(team, Ranger)
  let hero = game.world.heroes[team.ord * 5]
  discard game.decide()
  game.world.footmen = @[Footman(id: 9000, team: Team(1-team.ord),
    hp: FootmanHp, position: game.world.forts[team.ord].center, state: Marching)]
  discard game.decide()
  let vm = game.heroVms[team.ord * 5]
  doAssert vm.runtime.getGlobal("homeThreat") > 0
  game.world.footmen.setLen(0)
  game.world.tick += 24
  let released = game.decide()
  doAssert vm.runtime.getGlobal("homeThreat") == 0
  doAssert released.has(ActionAttackMove)
  doAssert vm.runtime.getGlobal("goalX") != vm.runtime.getGlobal("homeX") or
    vm.runtime.getGlobal("goalY") != vm.runtime.getGlobal("homeY")
  inc checks

  hero.place(middle())
  game.world.footmen = @[Footman(id: 9001, team: Team(1-team.ord),
    hp: FootmanHp, position: middle(), state: Marching)]
  discard game.decide()
  hero.attacksLanded += 1
  let afterHit = game.decide()
  doAssert afterHit.has(ActionWalkTo) and not afterHit.has(ActionAttackTarget)
  doAssert vm.runtime.getGlobal("resumeTarget") > 0
  let resumed = game.decide()
  doAssert resumed.has(ActionAttackTarget)
  doAssert vm.runtime.getGlobal("resumeTarget") == 0
  inc checks

  for cell in game.world.teamVisible[team.ord].mitems: cell = 0
  hero.attackObjectId = 0
  doAssert not game.decide().has(ActionAttackTarget)
  doAssert vm.runtime.getGlobal("bestId") == 0
  inc checks

echo $(%*{"passed": true, "checks": checks,
  "max_instructions": maximumInstructions, "max_work": maximumWork,
  "scope": "All ten classes on both colors: real host upgrades, shop, portal guards, death/buyback, 600-creep stress; both-color finite defense release, observed-hit fractional movement and fog exclusion."})
