' Generated from gota_formation3600_ally_under_attack_v3; gota-semantic-policy/1; gota-basic/1
    ...
    while mIndex < objectCount()
      ...
      if objectHp(mIndex) > 0 then
        ...
        if objectTeam(mIndex) <> selfTeam and (mKind = 2 or mKind = 3 or mKind = 4) then
          ...
          if mKind = 2 and mD <= 6 * 6 and mTD <> 0 and objectTarget(mIndex) = mTD and objectHp(mIndex) < mTryY then
            mTryX = mId
      ...
      mIndex = mIndex + 1
    wend
    if mTryX <> 0 and mAlly = mTD and mAllyDistance <= 36 and (bestId = 0 or mSepX = 3) and selfHp * 100 >= selfMaxHp * 25 then
      bestId = mTryX
    end if
