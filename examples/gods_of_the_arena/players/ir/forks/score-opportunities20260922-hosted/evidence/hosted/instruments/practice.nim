## Public target-selection fixtures; counterpart VM is deliberately absent.
...
let enforce=getEnv("SCORE_ENFORCE")=="1"
var rows=newJArray()
...
for team in Team:
  for class in HeroClass:
    for scene in ["finishing_hero","healthy_hero","bounded_chase","disadvantage","hero_over_fort","tower_guard","retreat","only_fort","finish_god"]:
      ...
      if scene=="hero_over_fort":enemy.hp=class.heroDamage(h.level)*3
      if scene=="bounded_chase":
      ...
      let expected=if scene in ["finishing_hero","hero_over_fort"]:enemy.id
                   elif scene in ["only_fort","finish_god"]:g.world.forts[1-team.ord].id
      ...
      if scene in ["finishing_hero","finish_god"]:
        for tick in 0..<96:g.step()
      let realized=(scene!="finishing_hero" or h.totalXp>=150) and (scene!="finish_god" or h.totalXp>=500)
...
echo $(%*{"checks":rows.len,"rows":rows,"max_instructions":maxInstructions,"max_work":maxWork,"scope":"Artificial fully visible host fixtures isolate target choice and realized150XP finishing reward; not competitive evidence."})
