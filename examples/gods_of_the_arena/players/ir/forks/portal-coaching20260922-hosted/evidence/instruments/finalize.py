...
def main():
    ...
    if report:
        p['belief']['claims']['CompetitiveGain']['claim']=f"The fixed160-game fresh-control study passed its prospective >=10% mean-score gain and >=95% each-color preservation gate: {report['aggregate_gain_percent']:.2f}% overall. The95% bootstrap gain interval is{report['aggregate_gain_ci95_percent']}; this does not establish confident generalization or league rank."
        controls,candidates=report['cells'][:2],report['cells'][2:]
        old_home=sum(c['portal']['home_portals_from_keep'] for c in controls)
        new_home=sum(c['portal']['home_portals_from_keep'] for c in candidates)
        p['belief']['claims']['CoachingFidelity']['claim']+=f' In160 fresh audited hosted games, keep-to-home portal starts are{new_home} in80 candidate games versus{old_home} in80 controls. Hosted score improvement is assessed separately.'
        p['belief']['claims']['CoachingFidelity']['evidence'].append({'artifact':'evidence/hosted/report.json'})
        p['belief']['claims']['KhorsSuperiority']={
            ...
            'status':'supported' if report['khors_superiority_passed'] else 'requires_review',
    ...
    if report and not report['khors_superiority_passed']:p['update']['needs_review'].append('belief/KhorsSuperiority')
    ...
    for name in ('duration-analysis.json','portal-residual-review.json'):
        if (hosted_folder/name).exists():copy(hosted_folder/name,out/'evidence/hosted'/name)
...
''')
    if report:
        lines=['\n## Hosted comparison\n',
               f"All160 games passed source/runtime/replay/integer-score audits: {all(c['invalid']==0 for c in report['cells'])}. Aggregate score gain: **{report['aggregate_gain_percent']:.2f}%**, bootstrap95% interval **{report['aggregate_gain_ci95_percent']}**. Frozen score gate: **{report['score_gate_passed']}**. Separate confident khors-superiority gate: **{report['khors_superiority_passed']}**.\n",
               '| Source | Color | Own mean score | khors mean score | Individual outscore count | Team wins | Keep-to-home starts |',
               '|---|---|---:|---:|---:|---:|---:|']
        for c in report['cells']:
            lines.append(f"| {c['name']} | {'blue' if c['side'] else 'red'} | {c['score']:.2f} | {c['khors_score']:.2f} | {c['individual_score_wins']}/40 | {c['team_wins']}/40 | {c['portal']['home_portals_from_keep']} |")
        lines.append('\nFixed mixed roster; draft outcomes vary. The result does not establish universal field strength or a #1 leaderboard guarantee. The source stays unchanged between local and hosted validation. New stats-guided XP-allocation hypotheses remain separate from this candidate.\n')
