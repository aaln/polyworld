POLICY = {'schema': 'gota-semantic-policy/1',
 'id': 'gota_explicit_sustain20260923',
 'situation': {'grounded': {'observation': 'Bassy Q16.16 decimals for fractional action coordinates, integer '
                            ...
                            'predicates': {'always': 'Unconditional lifecycle phase',
                                           ...
                                           'active': 'Alive, battle started, decision due, not channeling or '
                                                     'stunned',
                                           'active_resource_hero': 'Active Vanguard/DeathKnight or '
                                                                   'mana-restoring caster, preserving the '
                                                                   'existing portal/death/stun/draft lock.',
                                           'active_lane_healer': 'Active Druid or Vanguard/DeathKnight; '
                                                                 'carry and mana-only caster routing stays '
                                                                 'unchanged.',
                                           ...
                                           'recovery_remaining': 'Recovery owns the decision and no '
                                                                 ...
                                                                 'has claimed it.',
                                           'active_melee': 'Active melee hero under the normal '
                                                           'channel/death/stun/draft guard.'}},
               'notes': 'Coached home recall differs from outbound tower travel. in_base=canShop, '
                        ...
                        'regeneration is assumed. Release60 removes automatic spells. Missing HP/mana, '
                        'learned ability rank, cooldown, charge, public threats and shopping opportunities '
                        'jointly determine recovery. Resource spells unlock from level2 after the normal '
                        'level1 primary. Nearby hero pressure and local relative level-power trigger short '
                        'melee disengagements instead of long chases or unconditional home trips.'},
 'belief': {'grounded': {'memory': ['crossed',
                         ...
                         'uncertainty': 'Unseen enemies are unknown. Visible targets may be masked. Only the '
                                        ...
                                        'choices.'},
            'claims': {'HistoricalWeakHeroes': {'status': 'supported',
                                                'claim': 'Earlier replay59 cohort had100carry appearances '
                                                         'averaging3311.42 versus100melee averaging118.23. '
                                                         'This identifies a research priority, not '
                                                         'current-release performance or a causal class '
                                                         'comparison.',
                                                'evidence': [{'artifact': 'evidence/prior-evidence.json'}]},
                       'ExplicitSustainMechanism': {'status': 'supported',
                                                    'claim': 'On exact replay60,220real-tick fixtures '
                                                             'validate useful no-target healing/restoration, '
                                                             'level2resource unlock, no casts at full '
                                                             'resources or empty/cooling slots, portal lock, '
                                                             'short threat disengagement and retained creep '
                                                             'XP. Vanguard/DeathKnight retain15XP while '
                                                             'saving70/36mana in the one-HP creep fixture. '
                                                             'Six complete matches for unaffected classes '
                                                             'have identical commands/terminal state; '
                                                             'final16native games pass replay hashes and '
                                                             'all10VM limits. Local checks do not prove a '
                                                             'league gain.',
                                                    'evidence': [{'artifact': 'evidence/practice.json'}]},
                       'CompetitiveGain': {'status': 'contradicted',
                                           'claim': 'Fresh400game replay60 gate passed=False. Mean '
                                                    'score1901.635 to1735.790,gain-8.721percent,95percent '
                                                    'interval[-20.27602945838769, 4.513999059070661]. '
                                                    'Late-draft survival/XP gate=False. See all '
                                                    'class/context/frequency results; no claim of universal '
                                                    'rank, isolated-component causality or superiority on '
                                                    'every hero.',
 ...
 'goal': {'Win': {'preference': 'No independent victory incentive. Enemy-god destruction supplies500 own XP '
          ...
          'Practice': {'preference': 'Secure feasible last hits and shared XP, buy useful permanent power, '
                       ...
                       'provenance': 'authored'},
          'survive_and_replenish': {'preference': 'Explicit useful self recovery before unnecessary home '
                                                  'travel; retain dangerous escapes, useful shopping, portal '
                                                  'channels and bounded safe recovery. Improve low-scoring '
                                                  'melee income without sacrificing carry performance.',
                                    'provenance': 'interpretation'}},
 'skill': {'portal_state': {'operator': 'weak_portal_state', 'parameters': {}},
           'draft': {'operator': 'weak_draft', 'parameters': {'think_ticks': 6}},
           'recovery_intent': {'operator': 'weak_recovery_intent', 'parameters': {}},
           'base_recovery_intent': {'operator': 'weak_base_recovery_intent', 'parameters': {}},
           'timing': {'operator': 'weak_timing', 'parameters': {}},
           'lifecycle': {'operator': 'weak_lifecycle',
                         'parameters': {'buyback_seconds': 25, 'buyback_reserve': 200}},
           'observe': {'operator': 'weak_observe', 'parameters': {'scan_limit': 96}},
           'economy': {'operator': 'weak_economy', 'parameters': {'shop_gold': 500}},
           'portal_context': {'operator': 'weak_portal_context', 'parameters': {}},
           'explicit_sustain': {'operator': 'weak_explicit_sustain', 'parameters': {}},
           'lane_recovery': {'operator': 'weak_lane_recovery', 'parameters': {}},
           'replenish': {'operator': 'weak_replenish', 'parameters': {}},
           'channel_town_scroll': {'operator': 'weak_channel_town_scroll', 'parameters': {}},
           'walk_to_base': {'operator': 'weak_walk_to_base', 'parameters': {}},
           'home_portal': {'operator': 'weak_home_portal', 'parameters': {}},
           'tower_safety': {'operator': 'weak_tower_safety', 'parameters': {'tower_hp': 65}},
           'xp_close': {'operator': 'weak_xp_close', 'parameters': {}},
           'melee_spacing': {'operator': 'weak_melee_spacing', 'parameters': {}},
           'combat': {'operator': 'weak_combat', 'parameters': {'reset_recovery': 1}},
           'advance': {'operator': 'weak_advance', 'parameters': {}}},
 'strategy': [{'id': 'R_portal_state', 'when': 'always', 'skill': 'portal_state', 'for': ['Score']},
              ...
              {'id': 'R_explicit_sustain',
               'when': 'active_resource_hero',
               'skill': 'explicit_sustain',
               'for': ['Score', 'survive_and_replenish']},
              {'id': 'R_lane_recovery',
               'when': 'active_lane_healer',
               'skill': 'lane_recovery',
               'for': ['Score', 'survive_and_replenish']},
 ...
 'execution': {'binding': 'gota-bassy/explicit-sustain-2026-09-23-r60e',
               'game_version': '2026.9.23.2',
               'language': 'BASIC'},
 'update': {'revision': 2,
            'parent': '2bb5c5aa44c03abd4c4dde9a160255bd7434af3e9c677b5b24901128fcfbc01f',
            'change': {'origin': 'Completed local and fresh hosted evidence reflected into semantic IR; '
                                 'exact tested BASIC bytes retained.',
                       'deployment_qualified': False},
            'needs_review': [],
            'evidence': [{'artifact': 'evidence/trial-report.json'},
                         {'artifact': 'evidence/local-summary.json'},
                         {'artifact': 'evidence/local-exploration.json'}]}}
