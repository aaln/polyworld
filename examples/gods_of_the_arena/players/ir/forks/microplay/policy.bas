' Generated from gota_microplay_combined_v4; gota-semantic-policy/1; gota-basic/1
...
if objectCount() <= 48 then
  ...
  mpPick = 0
  mpScore = 2147483647
  mpRange = selfAttackRange * 60 / 60000
  mpIndex = 0
  while mpIndex < objectCount() and mpIndex < 64
    ...
    if 1 = 1 and objectKind(mpIndex) = 2 then
      if objectTeam(mpIndex) = selfTeam and objectAlive(mpIndex) and objectHp(mpIndex) > 0 then
        ...
        if mpDx * mpDx + mpDy * mpDy <= 8 * 8 then
      ...
      if objectKind(mpIndex) = 2 and objectTeam(mpIndex) <> selfTeam then
        if objectAlive(mpIndex) and objectHp(mpIndex) > 0 then
          ...
          if mpBoundX * mpBoundX + mpBoundY * mpBoundY <= mpRange * mpRange then
            mpRank = 0
            if 1 = 1 and objectId(mpIndex) = mpAssist then
              if mpSelfThreat = 0 or mpLeaderHp * 2 < selfHp then
              mpRank = 1
            end if
            end if
            if 1 = 1 and objectHp(mpIndex) <= selfAttackDamage then
              mpRank = 2
            end if
            if mpRank > 0 then
