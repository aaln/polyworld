' Generated from gota_reward_finish20260922; gota-semantic-policy/1; gota-bassy/reward-finish-2026-09-22-r1
...
if active = 1 then
  ...
  if dx * dx + dy * dy < 144 then
          ...
          if distance <= 324 and objectAlive(idx) = 1 then
            ...
            if hp <= selfAttackDamage and distance <= finishReach * finishReach then
              if kind = 2 then
                score = score + 1000
              end if
