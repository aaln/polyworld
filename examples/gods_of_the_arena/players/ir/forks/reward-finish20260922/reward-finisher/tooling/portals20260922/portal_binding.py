...
BINDING = 'gota-bassy/portals-2026-09-22-r3'
...
homeAnchorDistance = 1000000''')
    ...
    marker = '  if healCount = 0 and budget >= 30 and empty > 0 then'
    economy = economy.replace(marker, '''  if gearCount >= 4 and budget >= 100 then
    reserveSlot = 0
    while reserveSlot < 6
...
''', 'Hold the channel lock through its zero-remaining-ticks completion boundary; release only when the host publishes cooldown (completed/interrupted) or death.', memory=('portalBusy',))
    for name in ('draft','timing'):
        specs[name] = replace(specs[name], template=specs[name].template.replace('if selfChannelTicks = 0 and', 'if portalBusy = 0 and selfChannelTicks = 0 and'))
    specs['base_recovery_intent'] = spec('''
retreat = 1
resumeTarget = 0
''', 'Low health inside the keep still commits to walking toward the fountain; never leave base merely because field-only recall is ineligible.', memory=('retreat','resumeTarget'))
...
''', 'When recall is unsafe/unavailable, or already inside the keep, walk toward spawn and reconsider on later decisions. Interrupted channels use live cooldown/root state; no retry loop spends extra charges.', memory=('moveTick',), actions=('walkTo',))
    ...
    for name in ('channel_town_scroll','home_portal','advance'):
        specs[name] = replace(specs[name], template=specs[name].template.replace('stopped = 1', 'stopped = 1\nportalBusy = 1'), memory=specs[name].memory+('portalBusy',))
    del specs['recover']
    order = ['portal_state','draft','recovery_intent','base_recovery_intent','timing','lifecycle','observe','economy','portal_context','replenish','channel_town_scroll','walk_to_base','home_portal','tower_safety','xp_close','combat','advance']
    ...
    binding.PREDICATES.update({
        'low_health_in_field': ('drafting = 0 and selfHp > 0 and portalBusy = 0 and selfChannelTicks = 0 and selfStunTicks = 0 and canShop() = 0 and selfHp * 100 < selfMaxHp * 30', (), 'Below 30% health outside the friendly keep/spawn, alive and not channeling/stunned. Recovery takes priority over farming and attack reacquisition.'),
        'low_health_in_base': ('drafting = 0 and selfHp > 0 and portalBusy = 0 and selfChannelTicks = 0 and canShop() = 1 and selfHp * 100 < selfMaxHp * 30', (), 'Critical health inside own keep/spawn commits to walking and replenishment without a home portal.'),
...
def main():
    ...
    predicates = {'recovery_intent':'low_health_in_field','base_recovery_intent':'low_health_in_base','channel_town_scroll':'town_scroll_ready','walk_to_base':'recovery_remaining','replenish':'in_base'}
    p['strategy'] = [{'id':'R_'+k,'when':predicates.get(k,'always' if k in ('portal_state','draft','timing','lifecycle') else 'active'),'skill':k,'for':['Grow','Survive','Score','Practice','survive_and_replenish']} for k in specs]
