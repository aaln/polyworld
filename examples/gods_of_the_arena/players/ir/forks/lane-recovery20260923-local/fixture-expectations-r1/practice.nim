...
for team in Team:
  for scene in ["ready", "cooldown_three_seconds", "no_mana", "no_charges", "long_cooldown", "near_hero", "near_creep", "shop_affordable", "shop_unaffordable", "full_inventory", "rooted", "recovered", "low_mana_recovered", "warning", "warning_overflow", "timeout"]:
    ...
    for tick in 0..<duration:
      ...
      g.step()
      if tick==0:firstHeld=g.heroVms[slot].runtime.getGlobal("laneUntil")>g.world.tick
    ...
    if candidate and scene=="shop_affordable":passed=passed and homeWalks>0 and casts==0 and g.heroVms[slot].runtime.getGlobal("restock")==1
...
for team in Team:
  for class in HeroClass:
    for scene in ["potion_recovery","no_heal_source","recovered"]:
      ...
      let released=g.heroVms[slot].runtime.getGlobal("retreat")==0
      let expected=candidate and scene!="no_heal_source"
      let passed=released==expected and (not expected or (advances>0 and homeWalks==0)) and (scene!="potion_recovery" or heals>0)
      let row= %*{"scene":scene,"side":team.ord,"class":class.ord,"retreat_released":released,"healing":heals,"home_walks":homeWalks,"advances":advances,"expected_release":expected,"passed":passed}
      rows.add(row)
      if not passed:stderr.writeLine($row)
var allPassed=true
for row in rows:
