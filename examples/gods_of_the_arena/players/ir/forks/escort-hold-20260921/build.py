"""Fork formation-adaptive-20260920 with H-ESCORT-HOLD-01; never patch generated BASIC.
...
NEW_REL = 'if worldTick <> lastJoinTick + 1 or worldTick - worldTick / 48 * 48 = 0 then\nmoveAccepted = attackMove(moveX, moveY)\nif moveAccepted = 0 then\nescortId = 0\nmoveAccepted = walkTo(64, 64)\nend if\nend if\n'
...
def main():
    ...
    p['update'].update(revision=p['update']['revision']+1, parent=digest(parent),
        change={'origin': 'Devin cycle 12 fork for Richard/relh trigger (farm-rate mechanism)', 'hypothesis': 'H-ESCORT-HOLD-01 (route escort walkTo -> attackMove issued on entry and every 48 ticks only, so engine acquisition can complete basic hits; zero new globals)',
    ...
    out.rmdir()
    (HERE/'policy.py').write_text('"""H-ESCORT-HOLD-01 fork of the adaptive formation3600 IR; untested until README says otherwise."""\nPOLICY = '+pprint.pformat(p,width=110,sort_dicts=False)+'\n')
    ...
    cm['interpretation'] = 'Parent frozen compiler modules; contracts add formation_profile_route_escort_attackmove_hold_v1 (template-level, no BASIC patch).'
