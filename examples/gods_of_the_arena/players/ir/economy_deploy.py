...
from economy_hosted import OPTIMIZER
...
MOTION = 'ed2596c6-f9f7-4fe2-8b13-9383c0be7ba0'
...
def readiness(directory):
    ...
    source = (directory / 'economy/candidates' / name / 'policy.bas').read_text()
    ...
    if (plan['sample_size'] != 400 or
            any(result['arms'][arm]['games'] != 400 for arm in ['v2', 'motion', name]) or
...
def owned_champions(rows, candidate):
    ...
    if (by_player[AARON]['policy_version']['id'] != CONTROL or
            by_player[OPTIMIZER]['policy_version']['id'] not in {MOTION, candidate}):
