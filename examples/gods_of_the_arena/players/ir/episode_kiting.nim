...
while game.run.world.tick < options.maximumTicks and not game.run.world.gameOver:
  ...
  for slot, vm in game.run.heroVms:
    ...
    if hero.damageLanded and not wasLanded[slot] and hero.state == Fighting:
      inc basicHits[slot]
      lastHitTick[slot] = int(game.run.world.tick)
    wasLanded[slot] = hero.damageLanded
    let burst = optionalGlobal(vm.runtime, "kiteBursts")
    if burst.kind != JNull:
      if burst.getInt > lastBurst[slot] and vm.runtime.getGlobal("kiteTrigger") == 1:
        if basicHits[slot] > 0 and int(game.run.world.tick) - lastHitTick[slot] <= 18:
          inc verifiedBursts[slot]
        else:
          inc unverifiedBursts[slot]
      lastBurst[slot] = int32(burst.getInt)
