...
var
  ...
  commands: array[10, array[15, int]]
    ...
    if firstGearTick[slot] < 0:
      for item in hero.inventory:
        if item.ord > 4:
          ...
          break
    previous[slot] = hero.position
  if run.world.tick mod 240 == 0:
    var frame = newJArray()
    for slot, hero in run.world.heroes:
      frame.add(%*{"slot":slot,"x":hero.position.x,"z":hero.position.z,
        "hp":hero.hp,"max_hp":hero.maxHp,"mana":hero.mana,"level":hero.level,
        "xp":hero.xp,"gold":hero.gold,"inventory":hero.inventory,
        "target":hero.attackObjectId,"charges":hero.charges,"cooldowns":hero.cooldowns})
    frames.add(%*{"tick":run.world.tick,"heroes":frame})
...
echo $(%*{"ticks": run.world.tick, "hash_mismatches": run.hashCheck.mismatches,
  ...
  "state_hash": run.stateHash().toHex(16), "heroes": heroes, "events": events, "frames":frames})
