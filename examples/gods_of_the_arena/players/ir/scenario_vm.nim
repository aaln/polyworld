...
const dataNames = ["selfId", "selfTeam", "selfClass", "selfX", "selfY",
  "selfHp", "selfMaxHp", "selfMana", "selfMaxMana", "selfGold", "selfLevel", "worldTick", "selfAttackCooldown", "selfAttacksLanded", "selfAttackRange", "selfMoveSpeed", "selfAttackDamage"]
...
for name in ["walkTo", "attackTarget", "buyItem", "useItem", "castTarget"]:
  discard host.addFunction(name, (if name == "walkTo" or name == "castTarget": 2 else: 1), command(name),
    (if name == "walkTo": 800 else: 20))
...
for fixture in request["decisions"]:
  ...
  output.add(%*{"actions": actions, "memory": memory,
    "instructions": stats.instructions, "work": stats.workUnits})
echo $output
