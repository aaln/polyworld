' Generated from gota_duelist_reserve_consumables; gota-semantic-policy/1; gota-basic/1
...
if emptySlot <> 0 then
  ...
  if melee = 1 then
    if hasGear = 0 then
      if selfGold >= 70 then
        if gearCount < 4 then
          gearBought = buyItem(7)
          if gearBought <> 0 then
            gearCount = gearCount + 1
          end if
        end if
      end if
