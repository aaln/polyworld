' Generated from gota_waveguard_v2_burst_kite_20260915; gota-semantic-policy/1; gota-basic/1
...
if bestId <> 0 and kiteThreat <> 0 and worldTick >= kiteReady then
  if selfHp * 100 < selfMaxHp * 35 and kiteThreatDistance <= 36 and selfHp < kiteLastHp then
    kiteTrigger = 2
  end if
  if kiteTrigger = 0 and kiteRanged and (kiteTargetKind = 2 or kiteTargetKind = 3) and kiteStillTicks >= 18 and kiteThreatDistance <= 3 * 3 then
    kiteTrigger = 1
  end if
end if
