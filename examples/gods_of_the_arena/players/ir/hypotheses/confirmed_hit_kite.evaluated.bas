' Generated from gota_waveguard_v2_confirmed_hit_kite_20260915; gota-semantic-policy/1; gota-basic/1
...
if bestId <> 0 and kiteThreat <> 0 then
  if worldTick >= kiteReady and worldTick >= kiteUntil and selfHp * 100 < selfMaxHp * 35 and kiteThreatDistance <= 36 and selfHp < kiteLastHp then
    kiteTrigger = 2
  end if
  if kiteTrigger = 0 and kiteRanged and (kiteTargetKind = 2 or kiteTargetKind = 3) and selfAttacksLanded > kiteLastHits and selfAttackCooldown > 0 and kiteThreatDistance <= 3 * 3 then
    kiteTrigger = 1
  end if
end if
