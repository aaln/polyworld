---
{
  "id": "2026-09-10-single-heal",
  "policy": "GOTA semantic IR",
  "baseline": "local Duelist SHA256 7247acb6fe63d5f0eb40dd21efe8e27263af06705bb7161faba362cc3d0e13f3",
  "candidate": "local gota_duelist_single_heal",
  "status": "inconclusive",
  "hypothesis": "The independent elixir and ration attempts can buy both in one decision, spending 80 gold and occupying two slots. Buying one may preserve equipment capacity, but can reduce immediate survival.",
  ...
  "evals": [
    ...
    "local:tmp/gota-ir/hypothesis-study-20260910/discovery/single_heal"
  ]
}
---

# Attempt a ration only if the elixir purchase did not succeed; preserve all other sustain rules.
...
Code location: `binding.py`, sustain / E1. Every other behavior is identical to the frozen Duelist parent; goals and claims are updated to describe the one change.

## Predictions — pre-registered

- If TRUE: Double-healing purchase attempts per decision decrease and equipment capacity improves; wins improve if the second healing stock is a net waste. Squishy ranged/magic classes may lose survival.
- If FALSE: Saved healing purchases do not improve capacity or wins, or lower emergency healing increases death rates.
...
Adversarial critique: Fewer purchases can indicate a poorer or shorter-lived hero rather than better economy. The controlled VM proves the return-sensitive branch. Compare completed game outcomes, per-decision double-heal attempts, and final equipment, acknowledging episode-length dependence. The design isolates this one change against Duelist; a direct comparison to the default alone would confound it with existing Duelist changes. A weak effect may remain unresolved at 120 independent seeds; do not extend N or loosen the rule after looking. Historical validation seeds are excluded. All VM failures invalidate candidate eligibility; interrupted artifacts are preserved and resume checks hashes. Eight local workers; no concurrent hosted batches.

## Result

Completed 60 games per arm on six discovery seed clusters, all ten classes separately. Candidate 30/60 versus matched Duelist 35/60; delta -8.33 percentage points. No incomplete or VM-failed games were scored. All source, replay and trace hashes passed audit.

Predeclared activation proxy: `{"candidate_rate": 0.024958953680465506, "control_rate": 0.01619489238009551, "observed": false}`.

Wins fell by five. The preregistered attempt-rate proxy increased, but it does not count accepted purchases. A post hoc audit of all ten classes on seed 61000 observed 14 double-success healing ticks for Duelist and zero for the candidate, with every tick hash matched. The branch works; competitive and equipment benefits were not demonstrated. This correction does not change the failed gate.

Per-class wins, deaths and equipment are preserved in [the report](../hypotheses-20260910-report.html) and [audited JSON](../hypotheses-20260910-result.json), SHA-256 `f82da4a2f319abf45c0c6184733fe0f639dd32d98b876d828824a86745e0405e`. The evaluated [IR](../hypotheses/single_heal.ir.json) generates [BASIC](../hypotheses/single_heal.bas) byte-for-byte identical to the tested source. Twenty-eight tests passed, including real-VM mechanics and both representation directions.
