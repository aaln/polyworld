...
var
  ...
  hasKite, checkedKite: array[10, bool]
  basicHits, lastHitTick, verifiedBursts, unverifiedBursts, movedKiteTicks: array[10, int]
  lastBurst: array[10, int32]
  previousPosition: array[10, WorldPoint]
  trace: File
...
while game.run.world.tick < options.maximumTicks and not game.run.world.gameOver:
  ...
  for slot, vm in game.run.heroVms:
    ...
    if hero.attacksLanded > basicHits[slot]:
      lastHitTick[slot] = int(game.run.world.tick)
    basicHits[slot] = int(hero.attacksLanded)
    if not checkedKite[slot]:
      hasKite[slot] = optionalGlobal(vm.runtime, "kiteBursts").kind != JNull
      checkedKite[slot] = true
    if hasKite[slot]:
      let burst = vm.runtime.getGlobal("kiteBursts")
      if burst > lastBurst[slot] and vm.runtime.getGlobal("kiteTrigger") == 1:
        if basicHits[slot] > 0 and int(game.run.world.tick) - lastHitTick[slot] <= 1:
          inc verifiedBursts[slot]
        else:
          inc unverifiedBursts[slot]
      lastBurst[slot] = burst
      if vm.runtime.getGlobal("kiteMoved") != 0 and hero.position != previousPosition[slot]:
        inc movedKiteTicks[slot]
    previousPosition[slot] = hero.position
...
for slot, hero in game.run.world.heroes:
  ...
  heroes.add(%*{"slot": slot, "team": hero.team.ord, "class": hero.class.ord,
    ...
    "basic_hit_events":basicHits[slot], "verified_bursts":verifiedBursts[slot],
    "unverified_bursts":unverifiedBursts[slot], "moved_kite_ticks":movedKiteTicks[slot],
    "kite_spell_casts":optionalGlobal(game.run.heroVms[slot].runtime,"kiteSpellCasts"),
    "kite_bursts":optionalGlobal(game.run.heroVms[slot].runtime,"kiteBursts"),
    "kite_escapes":optionalGlobal(game.run.heroVms[slot].runtime,"kiteEscapes"),
    "motion_completions":optionalGlobal(game.run.heroVms[slot].runtime,"motionCompletions"),
    "motion_failures":optionalGlobal(game.run.heroVms[slot].runtime,"motionFailures"),
    "tower_detours":optionalGlobal(game.run.heroVms[slot].runtime,"towerDetours"),
    "weapon_starts":optionalGlobal(game.run.heroVms[slot].runtime,"weaponStarts"),
    ...
    "siege_overrides":optionalGlobal(game.run.heroVms[slot].runtime,"siegeOverrides"),
