"""Evaluate any exact opponent version; preserve the original frozen Jordan evaluator."""
...
def main():
 ...
 for name in ['semantics.py','segment.py','evaluate_target.py']:
 ...
 for label in ['class','side']:
  ...
  robust[label]={'correct':sum(s['correct'] for s in scored),'n':len(scored),'accuracy':rate(sum(s['correct'] for s in scored),len(scored)),'by_context':{key:{'correct':sum(s['correct'] for s in scored if s['context']==key),'n':sum(s['context']==key for s in scored)} for key in {s['context'] for s in scored}}}
 ...
 for key,counts in sorted(frozen['models']['train']['contexts'].items()):
  ...
  mask=int(key.split('_')[1]);low=int(key.split('_')[-1]);pid=f"{plan['preference_prefix']}_O{mask*2+low+1:02d}"
  preferences.append({'id':pid,'when':key,'skill':chosen,'over':alternative,'n':len(paired),'chosen_count':count,'rate':rate(count,len(paired)),
   ...
   'base_rate':{'source':str(len(pop_ids))+'-trajectory population prior, same context and paired affordances','n':len(pop_pair),'chosen_count':sum(r['selected']==chosen for r in pop_pair),'rate':rate(sum(r['selected']==chosen for r in pop_pair),len(pop_pair))},
 ...
 for skill in SKILLS:
  ...
  skill_stats[skill]={'n':len(rows),'episodes':len({r['episode'] for r in rows}),'ticks':sum(r['duration_ticks'] for r in rows),'initiation_contexts':dict(Counter(context(r['situation']) for r in rows)),
   'termination':dict(Counter(r['termination'] for r in rows)),'left_censored':sum(r['predictor_tick'] is None for r in rows),'median_duration_ticks':statistics.median(r['duration_ticks'] for r in rows) if rows else None,
   'outcome_hp_change_median':statistics.median(r['outcome']['hp_change'] for r in rows) if rows else None,'concurrent_movement_ticks':dict(sum((Counter(r['concurrent_movement']) for r in rows),Counter())),
   'prediction':{'actual_n':len(actual),'predicted_n':len(pred),'recall':rate(sum(p['correct'] for p in actual),len(actual)),'precision':rate(sum(p['correct'] for p in pred),len(pred))},'example':rows[0]['id'] if rows else None}
 ...
 target_ids=train_ids+hold_ids;all_target=[r['id'] for r in plan['episodes'] if r['split']!='population'];visibility={}
 for label,ids in [('all_target',all_target),('distinct_train',train_ids),('heldout',hold_ids),('population',pop_ids)]:
 ...
 win_counts=Counter()
 for i in all_target:
  us=metadata[i]['observer_slot'];scores=results[i]['scores'];win_counts['opponent_win' if scores[5-us]>scores[us] else 'our_win' if scores[us]>scores[5-us] else 'draw']+=1
