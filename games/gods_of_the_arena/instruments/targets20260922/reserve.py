...
VERSION='gota-bassy/microplay-2026-09-22-r3'
...
def economy(potions):
    ...
    for item,cost in order:
        ...
        if item in (1,21):condition += ' and has16 = 1'
        ...
        lines += ['    end if','  end if']
    if potions:lines += ['  if has16 = 1 and healCount > 0 and healCount < 2 and budget >= 30 then','    buyItem(1)','  end if']
    ...
    return contract('\n'.join(lines),{'shop_gold':(500,300,1200)},(),(),('buyItem','useItem'),
      ('Reserve potion and portal slots; buy dagger, armor, axe and crossbow instead of low-impact boots.' if potions else 'Reserve one portal slot; spend on five permanent items: dagger, armor, axe, crossbow and longsword.')+' Reserve gold for missing armor before purchasing or replenishing scrolls/potions; mark accepted gear immediately. Safely return to shop when useful equipment remains missing and gold is banked.',('restock',))
...
def build(name,mode):
    ...
    p['update']={'revision':3,'parent':parent,'change':{'origin':'User asked to keep going: observed low-income heroes buy consumables while lacking armor; test an armor gold reserve against the practiced parent','variant':mode},'needs_review':['belief/Microplay'],'evidence':[{'artifact':'games/gods_of_the_arena/experiments/2026-09-21-targets-microplay.md'}]}
