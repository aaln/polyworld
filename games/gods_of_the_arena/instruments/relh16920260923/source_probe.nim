## Current59 relh169 source-reveal audit. Exact opponent VMs replace their recorded commands;
  ...
  tickWorld(game.run, proc() =
    ...
    for offset in 0..<10:
      ...
      if not controlled[slot]:
        ...
        var draftState = newJObject()
        if wasDrafting:
          var legal, picks = newJArray()
          for c in 0..9:legal.add(%game.run.world.heroAvailable(c.int32))
          for h in game.run.world.heroes:
            picks.add(%*{"id":h.id,"team":h.team.ord,"class":game.run.world.draftedClass(h.id)})
          draftState = %*{"tick":game.run.world.tick,"slot":slot,"team":hero.team.ord,"legal":legal,"picks":picks}
        ...
        if actual != perSlot[slot]:
          ...
          quit(2)
        if wasDrafting and actual.len > 0:
          draftState["commands"] = actionsJson(actual)
          draftState["draft_best_class"] = %vm.runtime.getGlobal("draftBestClass")
          drafts.add(draftState)
        checked += actual.len
...
echo $(%*{"schema": "gota-source-reveal-runtime-audit/1", "slots": slots,
  ...
  "drafts": drafts, "first_examples": examples, "all_state_hashes_equal": true, "all_actions_consumed": true,
