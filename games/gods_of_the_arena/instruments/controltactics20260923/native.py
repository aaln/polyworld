...
def main():
    ...
    cases = [{'name': name, 'side': side, 'ordinal': ordinal, 'seed': seed} for name in ['baseline', 'control-tactics'] for side in range(2) for ordinal in [2, 4] for seed in [9230630, 9230631]]
    plan = {'cases': cases, 'own_source_sha256': sha(POLICY), 'candidate_source_sha256': sha(RAW / 'control-tactics/policy.bas'), 'upstream_source_sha256': sha(reference), 'scope': 'Sixteen complete games, eight fixed baseline controls and eight candidate games, one subject and nine current reference VMs. Matched seed/roster/slot/config with nine responsive current reference VMs; runtime evidence, not league qualification.'}
    ...
    def run(case):
        ...
        subject = POLICY if case['name'] == 'baseline' else RAW / 'control-tactics/policy.bas'
