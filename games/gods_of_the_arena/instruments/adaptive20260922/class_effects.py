"""Mechanism diagnostics on four lexically selected episodes per frozen screen cell."""
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--stage',default='screen');p.add_argument('--watch',action='store_true');a=p.parse_args();out=STUDY/a.stage
    ...
    for arm in plan['arms']:
        cell=read(out/arm['name']/str(arm['side'])/'result.json')
        for row in sorted(cell['rows'],key=lambda r:r['episode'])[:4]:
            cases.append({'name':arm['name'],'side':arm['side'],'slot':arm['own_slots'][0],'episode':row['episode'],'source_sha256':arm['source_sha256']})
    definition={'selection':'First4 lexical episode UUIDs per cell after all cells complete; rule chosen before detailed effect decoding. Retrospective diagnostic subset, no independent holdout/competitive gate.','cases':cases,'instrument_sha256':sha(Path(__file__).with_name('combat_audit.nim')),'binary_sha256':sha(STUDY/'bin/combat-audit')}
    path=out/'effects-plan.json'
    if path.exists():assert read(path)==definition
    else:write(path,definition)
    def decode(c):
        src=out/c['name']/str(c['side'])/'artifacts'/c['episode'];dst=out/'effects'/c['episode']/'audit.json'
        if not dst.exists():
            q=subprocess.run([str(STUDY/'bin/combat-audit'),'--replay',str(src/'replay.bin')],capture_output=True,text=True,timeout=900);assert q.returncode==0,q.stderr[-1200:]
    ...
    cells=[]
    for arm in plan['arms']:
        r=[x for x in rows if x['name']==arm['name'] and x['side']==arm['side']]
        keys=sorted({k for x in r for k in x['hero']['totals']})
        cells.append({'name':arm['name'],'side':arm['side'],'n':len(r),'mean_totals':{k:statistics.mean(x['hero']['totals'].get(k,0) for x in r) for k in keys},'classes':[x['hero']['class'] for x in r]})
