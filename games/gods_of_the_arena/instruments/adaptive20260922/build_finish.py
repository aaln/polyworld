...
def main():
    ...
    p['id']='gota_reward_finish20260922'
    p['execution']['binding']=binding.VERSION
    p['execution']['game_version']='2026.9.22.3'
    p['situation']['notes']+=' Public one-hit target finishes ordered by current direct XP reward; original long-range/general target and recovery paths; no hidden player identity or lifetime-XP input. Own lifetime XP is not exposed by this host BASIC API.'
    ...
    p['belief']['claims']={
        'OpportunityMechanism':{'claim':'In physically realized host scenes, reachable one-hit god/hero/building finishes outrank the finishing creep in that order and produce the expected actual XP. Multi-hit/out-of-range choices and all non-observation skills retain the baseline controller.','status':'requires_review','evidence':[{'artifact':'evidence/practice.json'}]},
    ...
    p['update']={'revision':1,'parent':parent_digest,'change':{'origin':'User: keep improving individual points as opponents change; Richard167 and khors114. Current clean replay diagnosis, broad spell-pressure screen and its failed later-draft confirmation. The class-conditioned screen also regressed. Current exact reward values motivate a new local finishing-priority hypothesis; fresh full-policy evaluation required.'},'needs_review':['belief/'+k for k in p['belief']['claims']],'evidence':[{'artifact':'evidence/experiment.md'}]}
