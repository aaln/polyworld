...
OUT=ROOT/'examples/gods_of_the_arena/players/ir/forks/combat-elixir20260923'
...
def main():
    ...
    for name in ['diagnosis-summary.json','local-summary.json','native-plan.json','native-result.json','runtime-provenance.json','start.json','conversion-proof.json','source-discovery.json','practice-provenance.json','skill-difference.json','prospective-experiment.md','baseline-practice.json']:
        shutil.copy2(STUDY/name,OUT/'evidence'/name)
    shutil.copy2(ROOT/'games/gods_of_the_arena/experiments/2026-09-23-combat-elixir.md',OUT/'evidence/experiment.md')
    ...
    for entry in trial['candidates']:
        ...
        p['belief']['claims']['OpportunityMechanism'].update(status='supported',claim='All240 actual-tick item/healing/economy scenes pass across ten classes and both colors. Baseline separately satisfies240 expectations for its original behavior, including no immediate combat healing. Eight full native games pass runtime/replay/XP checks. This validates immediate healing, cooldown, spawn, inventory and reserve guards, not comparative rival score.',evidence=[{'artifact':'evidence/practice.json'}])
