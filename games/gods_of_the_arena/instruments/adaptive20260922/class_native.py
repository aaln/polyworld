...
def run(case):
    ...
    out=STUDY/'native'/label/(str(side)+'-'+str(seat))/str(seed);out.mkdir(parents=True,exist_ok=True)
...
def main():
    cases=[('baseline',s,k,2) for s in [0,1] for k in [922610,922611]]+[('crossbow-only',s,k,seat) for s in [0,1] for k in [922610,922611] for seat in [0,2]]
