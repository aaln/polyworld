...
def build(stage):
    ...
    new=h.read(STUDY/'new-version-admission.json')['version']
    arm=next(a for a in plan['arms'] if a['name']=='baseline' and a['side']==0)
    first=min(h.read(out/'baseline/0/episodes.json'),key=lambda e:(e['job_index'],e['id']))
    spec=h.read(out/'baseline/0/artifacts'/first['id']/'spec.json')
    digest=spec['players'][arm['roster'].index(new)]['content_hash']
    assert len(digest)==64
    h.freeze(out/'new-source-fingerprint.json',{'version':new,'episode':first['id'],'job_index':first['job_index'],'source_sha256':digest,'rule':'Lowest job index in first control request; no outcome selection.'})
    hashes[new]=digest
    assert h.read(STUDY/'prior-trial-report.json')['candidates'][0]['score_gate_passed']
    cells=[]
    for arm,cell in zip(plan['arms'],verdict['cells']):
        ...
        result={'name':arm['name'],'side':arm['side'],'games':arm['games'],'invalid':sum(not r['valid'] for r in rows),'distinct_streams':len(unique),'duplicate_groups':[v for v in unique.values() if len(v)>1],'means':{k:mean(r[k] for r in rows) for k in keys},'score_ci95':ci([r['score'] for r in rows]),'by_class':{str(cls):{'n':sum(r['class']==cls for r in rows),**{k:mean(r[k] for r in rows if r['class']==cls) for k in keys}} for cls in sorted({r['class'] for r in rows})},'pooled_xp_per_minute':sum(r['xp'] for r in rows)/sum(r['minutes'] for r in rows),'team_wins':sum(r['win'] for r in rows),'opponents':comparisons,'rows':rows}
    ...
    result={'complete':True,'stage':stage,'games':plan['games'],'engine_commit':plan['engine_commit'],'game_version':plan['game_version'],'selected':selected,'candidates':candidates,'cells':[{k:v for k,v in c.items() if k!='rows'} for c in cells],'field_changes':changes,'deployment_qualified':stage=='refresh' and selected is not None and not changes['game_changed'] and not any(c['player_id'] in study.CRITICAL for c in changes['champion_changes']),'evidence_scope':'Fresh controls and unmatched random seeds, one subject, fixed mixed roster;10 source hashes/VM exits, full replay hashes, XP sources and integer scores checked. Side-stratified independent whole-game bootstrap; duplicate streams disclosed. Fresh160game current-Julia confirmation after400game score gate; game and principal-champion stability required; background drift reported. No pooling of intervals. No universal rank or per-component causality.'}
    h.write(out/'report.json',result);print(json.dumps({k:v for k,v in result.items() if k!='cells'}),flush=True)
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--stage',choices=['refresh'],default='refresh');p.add_argument('--watch',action='store_true');a=p.parse_args();out=STUDY/a.stage;total=h.read(out/'plan.json')['games']
    while True:
