...
def upload_coach(c,source,schema,note):
    out=OUT/'coach';meta={'name':'aaron-gota-buyback0923-coach','content_hash':h.sha(source),'size_bytes':len(source),'player_id':PLAYERS['coach'],'attributes':{},'tags':{'game':'gods_of_the_arena','game_version':h.VERSION,'engine_commit':h.COMMIT,'validation':note}}
...
def main():
    ...
    selected=confirmation['selected'];assert selected=='core-buyback' and confirmation['games']==400
    result=confirmation['candidates'][0];assert result['score_gate_passed']
    pair=ROOT/'examples/gods_of_the_arena/players/ir/forks/core-buyback20260923'/selected
    ...
    note=f"Post-core early buyback XP policy with time and gold guards. One preselected source,400fresh held-out games,100/source/color, fixed first team seat: gain{result['aggregate_gain_percent']:.3f}%, colors{result['per_color_gain_percent']},95% interval{result['gain_ci95_percent']}. All10source/VM/replay/XP/integer audits. ExactRichard174 andkhors114; no universal ranking guarantee. Prior user authorization for both players."
    placement.NOTE=note
