...
def main():
    pair=STUDY/'combat-elixir';proof=[]
    ...
    for name,count in [('practice',240),('portals',84)]:
        d=read(STUDY/('combat-elixir-'+name+'.json'))
        assert len(d['rows'])==count and all(r['passed'] for r in d['rows'])
    broad=read(STUDY/'combat-elixir-scenarios.json');assert broad['passed'] and broad['checks']==126
    native=read(STUDY/'native-result.json');assert native['passed'] and len(native['rows'])==8
    control=read(STUDY/'baseline-practice.json');assert len(control['rows'])==240 and all(r['passed'] for r in control['rows'])
    ...
    instrument=Path(__file__).with_name('elixir_practice.nim')
    engine=ROOT.parent/'polyworld-gota-engine-score-20260922'
    assert sha(instrument)==sha(engine/'examples/gods_of_the_arena/tools/elixir_practice.nim')
    for name,digest in runtime['engine_files'].items():assert sha(engine/name)==digest
    write(STUDY/'practice-provenance.json',{'engine_commit':runtime['engine_commit'],'instrument_sha256':sha(instrument),'binary_sha256':sha(STUDY/'practice'),'build_log_sha256':sha(STUDY/'build-practice-r2.log'),'initial_failed_capture':'build-practice.log','fixture_repair':'Remove assignment to computed abilityPoints accessor; policy bytes unchanged.','runtime_receipt':'runtime-provenance.json'})
    summary={'passed':True,'source_sha256':sha(pair/'policy.bas'),'target_reward_cases':240,'baseline_expectations_passed':240,'portal_cases':84,'broader_cases':126,'full_native_games':8,'max_fixture_instructions':max(read(STUDY/('combat-elixir-'+n+'.json'))['max_instructions'] for n in ['practice','portals','scenarios']),'max_fixture_work':max(read(STUDY/('combat-elixir-'+n+'.json'))['max_work'] for n in ['practice','portals','scenarios']),'competitive_inference':False}
