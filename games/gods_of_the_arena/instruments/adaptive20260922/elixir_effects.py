...
def main():
    ...
    definition={'selection':'First4 lexical episode UUIDs per cell after all cells complete; rule chosen before detailed effect decoding. Retrospective diagnostic subset, no independent holdout/competitive gate.','cases':cases,'instrument_sha256':sha(Path(__file__).with_name('elixir_audit.nim')),'binary_sha256':sha(STUDY/'elixir-audit')}
    ...
    def decode(c):
        ...
        if not dst.exists():
            q=subprocess.run([str(STUDY/'elixir-audit'),'--replay',str(src/'replay.bin')],capture_output=True,text=True,timeout=900);assert q.returncode==0,q.stderr[-1200:]
