...
def main():
    ...
    p['id']='gota_crossbow_only20260922'
    p['execution']['binding']=binding.VERSION
    p['execution']['game_version']='2026.9.22.3'
    p['situation']['notes']+=' Crossbow-only public spell target and hero priority; original action paths for all other classes; no hidden player identity or lifetime-XP input. Own lifetime XP is not exposed by this host BASIC API.'
    ...
    p['skill']={k:{'operator':'class_'+k,'parameters':v.defaults()} for k,v in specs.items()}
    old={x['id'][2:]:x for x in p['strategy']}
    p['strategy']=[dict(old.get(k,{'id':'R_'+k,'when':'active','skill':k}), **{'for':['Score']}) for k in specs]
    p['belief']['claims']={
        'OpportunityMechanism':{'claim':'Crossbowman alone uses the tested independent hero spell attempt and raised hero attack priority. All other classes retain the deployed controller action templates. Discriminating host scenarios must show intervention on Crossbowman and preserved actions for all nine other classes.','status':'requires_review','evidence':[{'artifact':'evidence/practice.json'}]},
    ...
    p['update']={'revision':1,'parent':parent_digest,'change':{'origin':'User: keep improving individual points as opponents change; Richard167 and khors114. Current clean replay diagnosis, broad spell-pressure screen and its failed later-draft confirmation. Class slices motivate a new hypothesis only; fresh full-policy evaluation required.'},'needs_review':['belief/'+k for k in p['belief']['claims']],'evidence':[{'artifact':'evidence/experiment.md'}]}
