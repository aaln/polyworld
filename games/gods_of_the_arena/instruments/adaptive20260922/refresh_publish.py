...
OUT=ROOT/'examples/gods_of_the_arena/players/ir/forks/reward-finish-refresh20260922'
read=lambda p:json.loads(p.read_text())
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
def write(p,v):p.parent.mkdir(parents=True,exist_ok=True);p.write_text(json.dumps(v,indent=2)+'\n')

def main():
    trial=read(STUDY/'refresh/report.json');assert trial['complete'] and trial['games']==160
    stages=['refresh']
    assert not OUT.exists(),'Preserve existing published inputs'
    OUT.mkdir(parents=True)
    for stage in stages:
        src=STUDY/stage;dst=OUT/'evidence'/stage
        for name in ['plan.json','result.json','report.json','field-changes.json','effects-plan.json','effects-summary.json','new-source-fingerprint.json']:
    ...
    for name in ['diagnosis-summary.json','local-summary.json','native-plan.json','native-result.json','runtime-provenance.json','start.json','conversion-proof.json','source-discovery.json','practice-provenance.json','prospective-experiment.md','baseline-practice-final.json','prior-trial-report.json','new-version-admission.json','metadata-reuse-proof.json','new-version-listing.json']:
        shutil.copy2(STUDY/name,OUT/'evidence'/name)
    shutil.copy2(ROOT/'games/gods_of_the_arena/experiments/2026-09-22-reward-refresh.md',OUT/'evidence/experiment.md')
    ...
    for entry in trial['candidates']:
        ...
        p['belief']['claims']['CompetitiveGain'].update(status='supported' if supported else 'contradicted' if not entry['score_gate_passed'] else 'requires_review',claim=f"Initial400games passed score gate (+12.438%,CI[0.084,26.409]) but failed all-field stability. 160-game fresh background-roster confirmation {'passed' if entry['score_gate_passed'] else 'failed'}: {entry['aggregate_gain_percent']:.3f}% aggregate, colors {entry['per_color_gain_percent']},95% interval {entry['gain_ci95_percent']}. Gate requires >=10%aggregate, >=95%eachcolor and zero invalid; initial400game gain bound positive. Game/principal-champion stability qualification: {supported}. Exact-version fixed first-seat roster only; no future rank or late-draft claim.",evidence=[{'artifact':'evidence/trial-report.json'}])
        p['belief']['claims']['FieldDrift']={'status':'supported','claim':'Opponent-version and game snapshots are evaluation metadata; newer entries can change optimal behavior. No opponent UUID or hidden replay state enters the live source. Principal-player or game changes prevent this confirmation deployment; other background changes are reported and exact tested-version scope is preserved.','evidence':[{'artifact':'evidence/field-changes.json'}]}
        ...
        shutil.copy2(STUDY/'refresh/report.json',evidence/'trial-report.json');shutil.copy2(STUDY/'refresh/field-changes.json',evidence/'field-changes.json')
        write(evidence/'parent-and-inputs.json',{'parent_pair':'examples/gods_of_the_arena/players/ir/forks/portal-coaching20260922-hosted','parent_source_sha256':sha(binding.PARENT/'policy.bas'),'original_local_pair':str(parent),'original_ir':read(parent/'policy.ir.json'),'raw_inputs':str(STUDY),'prior_400_pair':'examples/gods_of_the_arena/players/ir/forks/reward-finish20260922/reward-finisher','preserved_prior_failure':'examples/gods_of_the_arena/players/ir/forks/score-opportunities20260922-hosted','original_portal_coaching_session':'/Users/aaln/Documents/Policy Loops/sessions/2026-09-22t16-43-56-076z862811','explanation':'Original coaching captures and local semantic snapshots remain immutable. Parent evidence retains their original release scope.'})
...
'''
    ...
    write(OUT/'summary.json',{'refresh':trial['candidates'],'qualified':trial['selected'] if trial['deployment_qualified'] else None,'pairs':pairs})
