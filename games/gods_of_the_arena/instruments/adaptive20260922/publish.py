...
OUT=ROOT/'examples/gods_of_the_arena/players/ir/forks/adaptive-score20260922'
...
def main():
    ...
    for name in ['diagnosis-summary.json','local-summary.json','native-plan.json','native-result.json','runtime-provenance.json','start.json','conversion-proof.json','source-discovery.json']:
        shutil.copy2(STUDY/name,OUT/'evidence'/name)
    shutil.copy2(ROOT/'games/gods_of_the_arena/experiments/2026-09-22-adaptive-score.md',OUT/'evidence/experiment.md')
    ...
    for entry in screen['candidates']:
        ...
        p['belief']['claims']['OpportunityMechanism'].update(status='supported',claim='All44 actual-engine spell fixtures pass: independent nearby public hero casts produce real hero damage while basics select a finishing creep; out-of-range and allied alternatives fall back legally; useful mana restoration works. Recovery/channel fixtures remain protected. This does not establish competitive value.',evidence=[{'artifact':'evidence/practice.json'}])
