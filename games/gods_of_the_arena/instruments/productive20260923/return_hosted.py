"""Frozen eight-cell productive return comparison against the deployed Druid baseline."""
...
ROOT=base.ROOT
STUDY=ROOT.parent/'polyworld/tmp/gota-productive-return59-20260923'
base.STUDY=h.STUDY=panel.STUDY=audit.STUDY=STUDY
h.CYCLE='interactive-productive-return59-20260923'
h.GAME='cow_32f7afc6-78f6-49d5-990b-5b3c3619662c'
h.VERSION=audit.VERSION='2026.9.23.1'
h.COMMIT='d6827a4bd3a55a46cf86f88e921f147137709c64'
h.ENGINE=ROOT.parent/'polyworld-gota-engine-20260923'
...
def upload(c):
    data=source('productive-return').read_bytes();out=STUDY/'uploads/productive-return'
    meta={'name':'thistle-quartz-6a4d','content_hash':h.sha(data),'size_bytes':len(data),'player_id':h.PLAYER,'attributes':{},'tags':{'game':'gods_of_the_arena'}}
...
def prepare():
    ...
    assert h.sha(source('productive-return').read_bytes())==h.read(STUDY/'productive-return/manifest.json')['source_sha256']
    assert h.read(STUDY/'preflight.json')['passed']
    assert h.read(STUDY/'calibration-proof.json')['passed']
    out=STUDY/'trial'
    with h.research.lock(h.CAMPAIGN/'runner.lock',blocking=False),h.client() as c:
        ...
        versions={'baseline':BASELINE,'productive-return':upload(c)}
        ...
        for label,version in versions.items():
            for cell,side,ordinal in [('red-lead',0,0),('blue-lead',1,0),('red-late',0,3),('blue-late',1,3)]:
                ...
                body={'idempotency_key':f'gota-thistle-6a4d-{label}-{cell}-{digest[:10]}','target':{'coworld_id':h.GAME,'variant_id':'competition'},'game_config_overrides':cfg(game),'num_episodes':50,'roster':[{'slot':i,'player':{'policy_ref':v}} for i,v in enumerate(roster)],'notes':'Controlled policy evaluation.'}
        ...
        plan={'stage':'trial','games':400,'engine_commit':h.COMMIT,'game_version':h.VERSION,'cycle':h.CYCLE,'arms':arms,'rule':RULE,'budget':'3120/11600 reserved before;400new in8x50, separate cycle,maximum3active. User authorized10000additional games September23UTC.','source_evidence':'docs/opponents/khors-v114/score-audit-20260923','source_freeze':'Productive return r1, generated from current Druid IR; lane-aligned single-scroll outbound travel; fresh controls; natural draft.'}
