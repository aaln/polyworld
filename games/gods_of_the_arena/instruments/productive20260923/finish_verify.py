...
ROOT=Path(__file__).resolve().parents[4];RAW=ROOT.parent/'polyworld/tmp/gota-selective-finish59-20260923'
SOURCE=RAW/'selective-finish/policy.bas';PARENT=ROOT/'examples/gods_of_the_arena/players/ir/forks/druid-lane20260923-hosted/druid-lane/policy.bas'
read=lambda p:json.loads(p.read_text())
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
def save(p,d):p.write_text(json.dumps(d,indent=2)+'\n')
cases=[('finish',RAW/'finish-practice',{}),('practice',RAW/'return-practice',{'RETURN_CANDIDATE':'0','PORTAL_ENFORCE':'1'}),('baseline-practice',RAW/'return-practice',{'RETURN_CANDIDATE':'0','PORTAL_ENFORCE':'1'}),('recovery',RAW/'recovery-practice',{'LANE_CANDIDATE':'1','DRUID_ONLY':'1'}),('openings',RAW/'opening-practice',{'BLUE_CANDIDATE':'1'}),('buyback',RAW/'buyback-practice',{'BUYBACK_CANDIDATE':'1'}),('scenarios',RAW/'bin/scenarios',{})]
def one(case):
    name,exe,flags=case
