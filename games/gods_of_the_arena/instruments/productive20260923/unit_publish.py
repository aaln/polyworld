"""Seal the completed unit-farming result and reflect evidence into its semantic IR."""
from pathlib import Path
import json,hashlib,shutil,pprint,subprocess,sys
import unit_binding as b
ROOT=b.ROOT;RAW=ROOT.parent/'polyworld/tmp/gota-unit-farming59-20260923';OUT=ROOT/'examples/gods_of_the_arena/players/ir/forks/unit-farming20260923-hosted'
...
def main():
 ...
 pair=OUT/'unit-farming';shutil.copytree(RAW/'unit-farming',pair,ignore=shutil.ignore_patterns('__pycache__'))
 evidence=OUT/'evidence';evidence.mkdir()
 for n in ['local-summary.json','native-plan.json','native-result.json','runtime-provenance.json','conversion-proof.json','skill-difference.json','preflight.json','calibration-proof.json','harness-amendment.json']:
  shutil.copy2(RAW/n,evidence/n)
 shutil.copy2(RAW/'budget/authorization-permanent-100000.json',evidence/'budget-authorization.json')
 missing_logs=[]
 for n in ['unit.json','baseline-unit.json','practice.json','baseline-practice.json','recovery.json','openings.json','buyback.json','scenarios.json']:
 ...
 shutil.copy2(ROOT/'games/gods_of_the_arena/experiments/2026-09-23-unit-farming.md',pair/'evidence/experiment.md')
 ...
 p['goal']['Win']['preference']='Enemy-god destruction supplies 500 own XP and avoids later elapsed-time cost. This experiment forgoes deliberate structure targeting to test the opportunity cost against recurring unit XP; incidental area damage is still possible. Longer games alone do not improve score.'
 p['belief']['claims']['RecoveryMechanism'].update(claim='The inherited Druid recovery behavior previously passed its own scoped study. For this unit-targeting source, 92 recovery, 100 opening, 180 buyback, 84 portal and 126 broader host checks pass again on replay59; 16 complete responsive native matches validate runtime. The separate 80 target fixtures validate the new eligibility rule. Prior non-Druid command-equivalence claims describe the Druid parent change, not this all-class targeting intervention.',evidence=[{'artifact':'evidence/recovery.json'},{'artifact':'evidence/local-summary.json'},{'artifact':'evidence/native-result.json'}])
 building=effects['totals']['unit-farming']['building_attack_commands']
 p['belief']['claims']['UnitFarming'].update(status='supported' if building==0 else 'requires_review',claim=f"All 80 target-eligibility fixtures plus 582 inherited checks and 16 native games pass on replay59. In the 32-game preselected hosted diagnostic subset, candidate explicit building-target commands={building}, baseline={effects['totals']['baseline']['building_attack_commands']}. This validates the observed targeting mechanism only; routing and competitive value remain separate.",evidence=[{'artifact':'evidence/local-summary.json'},{'artifact':'evidence/effects-summary.json'}])
...
''')
 ...
 (OUT/'README.md').write_text(f"# Unit-farming comparison on replay59\n\nOnly deliberate target eligibility changes from incumbent29f6d7e6. Source1238ec73; four color/draft contexts,50games/source/context. Mean score gain{entry['aggregate_gain_percent']:+.2f}%; deployment qualified:{report['deployment_qualified']}.\n\n[Reviewed semantic IR/policy pair](unit-farming/README.md), [all metrics and verdict](evidence/report.json), [mechanism subset](evidence/effects-summary.json). Raw captures remain unchanged.\n")
