...
let candidate=getEnv("RETURN_CANDIDATE")=="1"
...
for team in Team:
  for class in [Ranger,Crossbowman]:
    for scene in ["field27","field20","keep20","spawn20","no_scroll","cooldown","rooted","near_creep","far_creep","near_hero","anchor_missing","outward_one","outward_two","stack_restock","interrupted","anchor_destroyed","ordinary_command","posthit_critical","warning_near","warning_far24","warning_overflow"]:
      ...
      var startHp,startMaxHp,startMana,startMaxMana:int32
      var portalTower,expectedTower:int32
      var anchorScore=high(int32)
      let vm=g.heroVms[slot]
      let centerX=mapTiles().int32 div 2
      let centerY=mapTiles().int32 div 2
      for b in g.world.buildings:
        if b.team==team and b.kind==TowerBuilding and b.hp>0:
          let dx=mapCoordinate(b.position.x,team)-centerX
          let dy=mapCoordinate(b.position.z,team)-centerY
          if dx*dx+dy*dy<anchorScore:
            anchorScore=dx*dx+dy*dy;expectedTower=b.id
      ...
      for tick in 0..<180:
        ...
        if h.portalEnds>0 and startTick<0:
          ...
          startHp=h.hp;startMaxHp=h.maxHp;startMana=h.mana;startMaxMana=h.maxMana;portalTower=h.portalTowerId
      ...
      let shouldStart=(candidate and scene in ["outward_one","spawn20"]) or scene in ["field27","field20","far_creep","outward_two","stack_restock","interrupted","anchor_destroyed","ordinary_command","posthit_critical","warning_far24"]
      var passed=(startTick>=0)==shouldStart and channelActions==0
      if shouldStart and scene!="spawn20":passed=passed and startTick<=40
      if candidate and scene=="spawn20":passed=passed and startHp*10>=startMaxHp*9 and startMana*10>=startMaxMana*8
      if candidate and team==BlueTeam and scene in ["outward_one","outward_two"]:passed=passed and portalTower==expectedTower
      ...
      if scene in ["outward_one"]:passed=passed and h.itemCounts[5]==(if candidate:0 else:1)
      ...
      let row = %*{"scene":scene,"side":team.ord,"class":class.ord,"start_tick":startTick,"complete_tick":completeTick,"started_in_keep":startedInKeep,"start_hp":startHp,"start_max_hp":startMaxHp,"portal_tower":portalTower,"expected_center_tower":expectedTower,"portal_commands":portalCommands,"channel_actions":channelActions,"walks":walks,"attacks":attacks,"scrolls_remaining":h.itemCounts[5],"bought_scrolls":boughtScrolls,"ordinary_rejected_without_cancel":ordinaryRejected,"passed":passed}
      rows.add(row)
