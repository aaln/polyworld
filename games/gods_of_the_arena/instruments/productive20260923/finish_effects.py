...
ROOT=Path(__file__).resolve().parents[4];RAW=ROOT.parent/'polyworld/tmp/gota-selective-finish59-20260923';OUT=RAW/'trial';BIN=RAW/'finish-opportunity'
...
definition={'selection':'First4lexical episode UUIDs per cell; rule preregistered before hosted outcomes. Diagnostic subset only.','cases':cases,'binary_sha256':sha(BIN),'source_sha256':sha(Path(__file__).with_name('finish_opportunity.nim'))}
...
summary={'plan':definition,'rows':rows,'totals':{n:{'games':sum(r['name']==n for r in rows),'building_attack_commands':sum(r['building_attack_commands'] for r in rows if r['name']==n),'unit_attack_commands':sum(r['unit_attack_commands'] for r in rows if r['name']==n)} for n in ['baseline','selective-finish']},'scope':'Actual submitted commands,32preselected games; not an independent competitive sample or source reconstruction.'}
