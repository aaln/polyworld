...
ROOT=Path(__file__).resolve().parents[4];RAW=ROOT.parent/'polyworld/tmp/gota-productive-return59-20260923'
SOURCE=RAW/'productive-return/policy.bas';PARENT=ROOT/'examples/gods_of_the_arena/players/ir/forks/druid-lane20260923-hosted/druid-lane/policy.bas'
read=lambda p:json.loads(p.read_text())
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
def save(p,d):p.write_text(json.dumps(d,indent=2)+'\n')
cases=[('practice',RAW/'return-practice',{'RETURN_CANDIDATE':'1','PORTAL_ENFORCE':'1'}),('baseline-practice',RAW/'return-practice',{'RETURN_CANDIDATE':'0','PORTAL_ENFORCE':'1'}),('recovery',RAW/'recovery-practice',{'LANE_CANDIDATE':'1','DRUID_ONLY':'1'}),('openings',RAW/'opening-practice',{'BLUE_CANDIDATE':'1'}),('buyback',RAW/'buyback-practice',{'BUYBACK_CANDIDATE':'1'}),('scenarios',RAW/'bin/scenarios',{})]
def one(case):
    name,exe,flags=case
    if (RAW/(name+'.json')).exists():
        d=read(RAW/(name+'.json'))
    else:
        p=subprocess.run([str(exe)],env=dict(os.environ,WEEK_POLICY=str(PARENT if name=='baseline-practice' else SOURCE),**flags),capture_output=True,text=True,timeout=600)
    ...
    ok=d.get('passed',all(r['passed'] for r in d.get('rows',[])));assert ok,name
...
def main():
    ...
    result={'passed':True,'source_sha256':sha(SOURCE),'rows':rows,'checks':sum(r['checks'] for r in rows if r['name']!='baseline-practice'),'native_games':16,'competitive_inference':False,'max_instructions':max(r['max_instructions'] for r in rows),'max_work':max(r['max_work'] for r in rows)}
