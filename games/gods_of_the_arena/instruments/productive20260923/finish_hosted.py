"""Frozen eight-cell selective-finish comparison against the deployed Druid baseline."""
...
h.CYCLE='interactive-selective-finish59-20260923'
...
def upload(c):
    data=source('selective-finish').read_bytes();out=STUDY/'uploads/selective-finish'
    meta={'name':'cedar-mica-72d8','content_hash':h.sha(data),'size_bytes':len(data),'player_id':h.PLAYER,'attributes':{},'tags':{'game':'gods_of_the_arena'}}
...
def prepare():
    ...
    assert h.sha(source('selective-finish').read_bytes())==h.read(STUDY/'selective-finish/manifest.json')['source_sha256']
    ...
    with h.research.lock(h.CAMPAIGN/'runner.lock',blocking=False),h.client() as c:
        ...
        versions={'baseline':BASELINE,'selective-finish':upload(c)}
        ...
        for label,version in versions.items():
            for cell,side,ordinal in [('red-lead',0,0),('blue-lead',1,0),('red-late',0,3),('blue-late',1,3)]:
                ...
                body={'idempotency_key':f'gota-cedar-72d8-{label}-{cell}-{digest[:10]}','target':{'coworld_id':h.GAME,'variant_id':'competition'},'game_config_overrides':cfg(game),'num_episodes':50,'roster':[{'slot':i,'player':{'policy_ref':v}} for i,v in enumerate(roster)],'notes':'Controlled policy evaluation.'}
        ...
        plan={'stage':'trial','games':400,'engine_commit':h.COMMIT,'game_version':h.VERSION,'cycle':h.CYCLE,'arms':arms,'rule':RULE,'budget':'Permanent100000/day authorized;400new in8x50, separate cycle, maximum3active; shared ledger retained.','source_evidence':'docs/opponents/khors-v179/score-audit-20260923','source_freeze':'Selective finish r1: bounded near-target eligibility and one-hit building priority; current Druid baseline; fresh controls; natural draft.'}
