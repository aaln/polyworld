"""Frozen eight-cell unit-farming comparison against the deployed Druid baseline."""
...
ROOT=base.ROOT
STUDY=ROOT.parent/'polyworld/tmp/gota-unit-farming59-20260923'
base.STUDY=h.STUDY=panel.STUDY=audit.STUDY=STUDY
h.CYCLE='interactive-unit-farming59-20260923'
...
def upload(c):
    data=source('unit-farming').read_bytes();out=STUDY/'uploads/unit-farming'
    meta={'name':'morrow-larch-84e1','content_hash':h.sha(data),'size_bytes':len(data),'player_id':h.PLAYER,'attributes':{},'tags':{'game':'gods_of_the_arena'}}
...
def prepare():
    ...
    assert h.sha(source('unit-farming').read_bytes())==h.read(STUDY/'unit-farming/manifest.json')['source_sha256']
    ...
    with h.research.lock(h.CAMPAIGN/'runner.lock',blocking=False),h.client() as c:
        ...
        versions={'baseline':BASELINE,'unit-farming':upload(c)}
        ...
        for label,version in versions.items():
            for cell,side,ordinal in [('red-lead',0,0),('blue-lead',1,0),('red-late',0,3),('blue-late',1,3)]:
                ...
                body={'idempotency_key':f'gota-morrow-84e1-{label}-{cell}-{digest[:10]}','target':{'coworld_id':h.GAME,'variant_id':'competition'},'game_config_overrides':cfg(game),'num_episodes':50,'roster':[{'slot':i,'player':{'policy_ref':v}} for i,v in enumerate(roster)],'notes':'Controlled policy evaluation.'}
