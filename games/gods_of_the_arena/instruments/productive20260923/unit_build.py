...
ROOT=b.ROOT;STUDY=ROOT.parent/'polyworld/tmp/gota-unit-farming59-20260923'
def write(p,v):p.parent.mkdir(parents=True,exist_ok=True);p.write_text(json.dumps(v,indent=2)+'\n')
def main():
 out=STUDY/'unit-farming';assert not out.exists(),'Preserve frozen candidate'
 specs=b.configure();ir=b.ir;p=json.loads((b.PARENT/'policy.ir.json').read_text());parent=ir.digest(p)
 p['id']='gota_unit_farming59_20260923';p['execution']['binding']=b.VERSION;p['execution']['game_version']='2026.9.23.1'
 p['skill']={k:{'operator':'unitfarm_'+k,'parameters':v.defaults()} for k,v in specs.items()}
 p['situation']['notes']+=' UnitFarming: only visible alive enemy heroes and creeps enter deliberate target ranking. Buildings remain observations for safety and navigation; hidden XP and opponent identity are never live inputs.'
 ...
 p['belief']['claims']['UnitFarming']={'status':'requires_review','claim':'In the coached khors179 game, all11892XP came from heroes and creeps. Coach spent18321damage on structures for500XP and then lost margin late. Removing buildings from deliberate selection may increase recurring XP, but can worsen routing, tower exposure, lost gold and objective rewards. The one selected high-score example is not causal or representative.','evidence':[{'artifact':'evidence/coaching-analysis.json'}]}
 ...
 p['update']={'revision':1,'parent':parent,'change':{'origin':'User coaching: Andre khors179 scores7781 while avoiding building attacks; test unit-only deliberate targets. Only observe executable changes; all other skills retained.'},'needs_review':['belief/UnitFarming','belief/CompetitiveGain'],'evidence':[{'artifact':'evidence/experiment.md'},{'artifact':'evidence/coaching-analysis.json'}]}
 ...
 shutil.copytree(b.PARENT/'tooling',out/'tooling',ignore=shutil.ignore_patterns('__pycache__'));(out/'tooling/productive20260923').mkdir();shutil.copy2(b.HERE/'unit_binding.py',out/'tooling/productive20260923/unit_binding.py')
 converter=(b.PARENT/'convert.py').read_text().replace('tooling/druidlane20260923','tooling/productive20260923').replace('import druid_binding\ndruid_binding.configure()\nir = druid_binding.ir','import unit_binding\nunit_binding.configure()\nir = unit_binding.ir');(out/'convert.py').write_text(converter)
 (out/'evidence').mkdir();shutil.copy2(ROOT/'games/gods_of_the_arena/experiments/2026-09-23-unit-farming.md',out/'evidence/experiment.md');shutil.copy2(b.PARENT/'evidence/coaching-review.json',out/'evidence/coaching-review.json')
