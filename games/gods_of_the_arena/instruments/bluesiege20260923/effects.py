...
def main():
 ...
 def select(arm):
  ...
  return [{'name':arm['name'],'cell':arm['cell'],'side':arm['side'],'slot':arm['own_slots'][0],'episode':row['episode'],'class':row['class'],'source_sha256':arm['source_sha256']} for row in sorted(cell['rows'],key=lambda r:r['episode'])[:8]]
 def decode(c):
  ...
  if not dst.exists():
   env={**os.environ,'AUDIT_KIND':'ours','AUDIT_TRANSFER':'1' if c['name']=='blue-druid-siege' else '0','AUDIT_POLICY':str(policy),'AUDIT_SLOTS':str(c['slot'])}
 ...
 # sample is still the exact prospectively specified first8 per complete cell.
 ...
 write(out/'effects-summary.json',{'plan':definition,'cells':cells,'rows':rows,'scope':'16-game descriptive source reconstruction; guards and submitted commands are not landed damage or causal score contribution. Full400-game score qualification remains separate.'})
