...
NAMES={'aaron':'morrow-jasper-6e52','coach':'ivory-cedar-82a6'}
...
def main():
    ...
    selected=confirmation['selected'];assert selected=='blue-druid-siege' and confirmation['games']==400
    result=confirmation['candidates'][0];assert result['score_gate_passed']
    pair=ROOT/'examples/gods_of_the_arena/players/ir/forks/blue-druid-siege20260923-hosted'/selected
    ...
    note=f"Blue-Druid-only guarded structure pressure and siege attacker response, generated from current semantic IR. One preselected source,400fresh games,200/source on blue later draft, with red/other-hero exact local command equivalence: gain{result['aggregate_gain_percent']:.3f}%, contexts{result['per_context_gain_percent']},95% interval{result['gain_ci95_percent']}. All10source/VM/fullreplay/XP/integer audits pass;16-game source reconstruction verifies mechanism. Fixed roster, no universal ranking or component causality. Prior user authorization for both players."
