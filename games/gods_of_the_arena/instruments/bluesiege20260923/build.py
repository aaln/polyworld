...
ROOT,STUDY=b.ROOT,b.ROOT.parent/'polyworld/tmp/gota-blue-druid-siege-20260923'
def write(p,v):p.parent.mkdir(parents=True,exist_ok=True);p.write_text(json.dumps(v,indent=2)+'\n')
def main():
    out=STUDY/'blue-druid-siege';assert not out.exists(),'Preserve frozen source'
    specs=b.configure();ir=b.ir;p=json.loads((b.PARENT/'policy.ir.json').read_text());parent=ir.digest(p)
    p['id']='gota_blue_druid_siege20260923';p['execution']['binding']=b.VERSION
    ...
    p['belief']['claims']['CompetitiveGain']={'status':'requires_review','claim':'The new blue-Druid-only source must qualify on400fresh blue later-draft games against current Druid source; broad-study blue Druid results only generate this hypothesis. Red and other heroes require complete local command equivalence.','evidence':[{'artifact':'evidence/trial-report.json'}]}
    p['update']={'revision':1,'parent':parent,'change':{'origin':'New scoped hypothesis after the broad Richard174 transfer failed: enable coordinated pressure and retaliation only for blue Druids. Old favorable slices are hypothesis generation only;400fresh games required.'},'needs_review':['belief/RichardSourceTransfer','belief/CompetitiveGain'],'evidence':[{'artifact':'evidence/opponent/provenance.json'}]}
    ...
    shutil.copytree(b.PARENT/'tooling',out/'tooling',ignore=shutil.ignore_patterns('__pycache__'));(out/'tooling/richard17420260923').mkdir();shutil.copy2(b.HERE.parent/'richard17420260923/pressure_binding.py',out/'tooling/richard17420260923/pressure_binding.py');(out/'tooling/bluesiege20260923').mkdir();shutil.copy2(b.HERE/'blue_siege_binding.py',out/'tooling/bluesiege20260923/blue_siege_binding.py')
    converter=(b.PARENT/'convert.py').read_text().replace('tooling/druidlane20260923','tooling/bluesiege20260923').replace('import druid_binding\ndruid_binding.configure()\nir = druid_binding.ir','import blue_siege_binding\nblue_siege_binding.configure()\nir = blue_siege_binding.ir');(out/'convert.py').write_text(converter)
    shutil.copytree(ROOT/'docs/opponents/richard-v174/source-audit-20260923',out/'evidence/opponent')
    rows=[json.loads(f.read_text()) for f in (ROOT.parent/'polyworld/tmp/gota-richard174-source-20260923/own-audits-r2').glob('*/result.json')];assert len(rows)==4 and all(x['all_state_hashes_equal'] for x in rows);write(out/'evidence/own-diagnosis.json',{'rows':rows,'opportunities':sum(x['counts'].get('safe_tower_opportunity_while_nonlethal_creep_selected',0) for x in rows),'scope':'Retrospective current-source diagnosis, not transfer evidence.'})
