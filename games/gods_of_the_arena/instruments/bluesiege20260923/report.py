...
RIVALS={'khors:v114':'145c01e0-0cbf-4e1e-8120-11b437175b91','Richard:v174':'5b854825-446a-4864-8dfc-bb3404d4fe81','Jordan:v411':'a15665be-4edf-4857-b23b-2888b4b49868'}
...
def build(stage):
    ...
    assert stage=='trial' and plan['games']==400 and len(plan['arms'])==2 and all(a['games']==200 for a in plan['arms'])
    ...
    for entry in candidates:
        after=by[entry['name']]
        entry['druid_exposure']={n:sum(c['by_class'].get('3',{}).get('n',0) for c in cells if c['name']==n) for n in ['baseline',entry['name']]}
        entry['unchanged_context_equivalence']=h.read(STUDY/'equivalence.json')['passed']
        entry['relative_gaps']={label:{'baseline':next(o['own_minus_rival'] for o in before[0]['opponents'] if o['label']==label),'candidate':next(o['own_minus_rival'] for o in after[0]['opponents'] if o['label']==label)} for label in ['khors:v114','Richard:v174']}
        entry['relative_preservation']=all(v['candidate']>=v['baseline'] for v in entry['relative_gaps'].values())
        entry['absolute_score_gate_passed']=entry['score_gate_passed']
        entry['score_gate_passed']=entry['score_gate_passed'] and min(entry['druid_exposure'].values())>=40 and entry['unchanged_context_equivalence'] and entry['relative_preservation']
    ...
    result={'complete':True,'stage':stage,'games':plan['games'],'engine_commit':plan['engine_commit'],'game_version':plan['game_version'],'selected':selected,'candidates':candidates,'cells':[{k:v for k,v in c.items() if k!='rows'} for c in cells],'field_changes':changes,'deployment_qualified':stage=='trial' and selected is not None and not changes['game_changed'] and not any(c['player_id'] in study.CRITICAL for c in changes['champion_changes']),'evidence_scope':'Fresh controls and unmatched random seeds, one subject, fixed mixed roster;10 source hashes/VM exits, full replay hashes, XP sources and integer scores checked. Context-stratified independent whole-game bootstrap; duplicate streams disclosed. One new scoped blue-Druid source;400fresh blue later-draft games,200/source. Prior rejected broad-study class/color slice generated this hypothesis only. Red and other hero command/world equivalence required locally. Later draft has two identical fixed reference teammates;>=40Druids/source required,all actual classes included. Mean score gaps against khors114 and Richard174 must not worsen. Positive lower95%gain bound; no universal rank or per-component causality.'}
