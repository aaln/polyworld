...
PRIOR={'aaron':'d9761f9c-c1cb-4e61-9850-d5a7ea2dd3ce','coach':'4dbada5e-8a62-4654-89ab-34cbea555bfe'}
placement.PRIOR=PRIOR

NAMES={'aaron':'nacre-velvet-7c2f','coach':'umbra-wren-93ad'}
...
def main():
    ...
    selected=confirmation['selected'];assert selected=='druid-lane' and confirmation['games']==400
    result=confirmation['candidates'][0];assert result['score_gate_passed']
    pair=ROOT/'examples/gods_of_the_arena/players/ir/forks/druid-lane20260923-hosted'/selected
    ...
    note=f"Druid-only safe lane healing with bounded cooldown wait, recovered-route release and affordable core shopping. Other hero controllers unchanged with local complete command equivalence. One preselected source,400fresh held-out games,100/source/color, natural later-draft ordinal3: gain{result['aggregate_gain_percent']:.3f}%, contexts{result['per_context_gain_percent']},95% interval{result['gain_ci95_percent']}. All10source/VM/replay/XP/integer audits. ExactRichard174 andkhors114; fixed later-draft roster scope, no universal ranking guarantee. Prior user authorization for both players."
