...
import numpy as np
ROOT=Path(__file__).resolve().parents[4];STUDY=ROOT.parent/'polyworld/tmp/gota-druid-lane-20260923'
...
for i,n in enumerate(names):
 y=[cells[n,s]['means']['score'] for s in ['red-late','blue-late']];ci=[cells[n,s]['score_ci95'] for s in ['red-late','blue-late']]
...
ax.set_xticks([0,1],['Red late','Blue late']);ax.set_ylabel('Mean individual points');ax.set_title('Score by context · 100 games per bar');ax.legend(fontsize=8);ax.spines[['top','right']].set_visible(False)
components=['hero_xp','creep_xp','building_xp','god_xp','time','clamp/round'];palette=['#9e4937','#438363','#a89246','#786794','#687e97','#a8a8a8']
labels=[];rows=[]
for n in names[1:]:
 for side in ['red-late','blue-late']:
...
fig.suptitle('GOTA Druid lane recovery · '+a.stage+' · 2026.9.22.3',fontsize=15)
