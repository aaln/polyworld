...
OUT = ROOT / 'examples/gods_of_the_arena/players/ir/forks/druid-lane20260923-local'
...
def main():
    ...
    for label in ['practice', 'openings', 'buyback', 'portals', 'scenarios']:
        shutil.copy2(STUDY / f'druid-lane-{label}.json', evidence / f'{label}.json')
    for name in ['non-druid-equivalence.json', 'ranged-equivalence.json', 'ranged-equivalence-plan.json', 'baseline-reuse.json']:
        shutil.copy2(STUDY / name, evidence / name)
    shutil.copy2(STUDY / 'trial/plan.json', evidence / 'hosted-plan.json')
    write(evidence / 'hosted-preparation.json', {
        'games': 400, 'submitted': 0, 'status': 'prepared_authorized',
        'inert_policy_version': read(STUDY / 'uploads/druid-lane/version.json'),
    ...
    policy['belief']['claims']['RecoveryMechanism'].update(
        ...
        claim='All 92 actual-tick recovery fixtures, 100 opening checks, 180 buyback checks, '
              '84 portal checks and 126 broad checks pass; 16 main native cases plus four targeted ranged matches pass; eight main controls are preserved local deterministic controls. All non-Druid command and terminal-state equivalence checks pass. '
...
sys.path.insert(0,str(P/'tooling/druidlane20260923'))
import druid_binding as b
b.configure();ir=b.ir
read=lambda p:json.loads(p.read_text())
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
