...
import json
import subprocess
from bootstrap import STUDY, ENGINE, ROOT
...
def run(case):
    label,side,seed=case
    out=STUDY/'native'/label/str(side)/str(seed);out.mkdir(parents=True,exist_ok=True)
    if (out/'result.json').exists():return read(out/'result.json')
    source=ROOT/'examples/gods_of_the_arena/players/ir/forks/portal-coaching20260922-hosted/policy.bas' if label=='baseline' else STUDY/'candidate-r3/policy.bas'
    binary=STUDY/'bin58/episode-v2';tape=out/'replay.bin'
    ref=ENGINE/'examples/gods_of_the_arena/players/base.bas'
    roster=[str(source) if i==side*5 else str(ref) for i in range(10)]
    ...
    try:
        ...
        hero=live['heroes'][side*5]
        assert hero['score']==max(0,hero['xp']*1440-200*live['ticks'])//1440
        q=subprocess.run([str(STUDY/'bin58/economy'),'--replay',str(tape)],capture_output=True,text=True,cwd=ENGINE,timeout=900)
        ...
        result={'name':label,'side':side,'seed':seed,'valid':True,'source_sha256':sha(source),'replay_sha256':sha(tape),'ticks':live['ticks'],'hero':hero,'all_hashes_equal':True}
    except Exception as exc:result={'name':label,'side':side,'seed':seed,'valid':False,'error':repr(exc)}
    write(out/'result.json',result);print(json.dumps(result),flush=True);return result


def main():
    cases=[(n,s,k) for n in ['baseline','candidate-r3'] for s in [0,1] for k in [922510,922511]]
    plan={'cases':cases,'engine':'1b70894436b7ffdcd0d421b6b32c2415c9c8bfde','reference':sha(ENGINE/'examples/gods_of_the_arena/players/base.bas'),'candidate':sha(STUDY/'candidate-r3/policy.bas'),'config':sha(STUDY/'game-config.json'),'scope':'One subject, nine responsive native reference VMs. Mechanism/runtime, not rival-policy comparison.'}
