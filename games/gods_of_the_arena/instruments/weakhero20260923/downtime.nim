...
while run.world.tick<run.replayData.hashes.len:
  ...
  for i,h in run.world.heroes:
    ...
    if alive:
      if run.world.battleTick mod TickRate == 0:
        counters[i].add("alive_position_samples")
        var nearCreep = false
        for creep in run.world.footmen:
          if creep.hp <= 0 or creep.state == Dying or creep.team == h.team or creep.navLayer != h.navLayer: continue
          let dx = int64(h.position.x) - creep.position.x
          let dz = int64(h.position.z) - creep.position.z
          if dx*dx + dz*dz <= int64(6*WorldScale)*int64(6*WorldScale): nearCreep = true
        if nearCreep: counters[i].add("within_creep_xp_range_samples")
  ...
  for e in run.world.events:
    ...
    if actor in 0..9:
      ...
      if e.kind==Healing:
        counters[actor].add("healing_caused",e.amount)
        if e.target.id == e.actor.id and e.cause == AbilityEffect:
          counters[actor].add("ability_self_healing", e.amount)
      if e.kind==ManaChanged and e.amount > 0 and e.cause == AbilityEffect:
        counters[actor].add("ability_mana_restored", e.amount)
