...
def main():
    ...
    cases = [{'name': name, 'side': side, 'ordinal': ordinal, 'seed': seed} for name in ['baseline', 'explicit-sustain'] for side in range(2) for ordinal in [0, 4] for seed in [9230610, 9230611]]
    plan = {'cases': cases, 'own_source_sha256': sha(POLICY), 'candidate_source_sha256': sha(RAW / 'explicit-sustain/policy.bas'), 'upstream_source_sha256': sha(reference), 'scope': 'Sixteen complete games, eight fixed baseline controls and eight candidate games, one subject and nine current reference VMs. Controls reused across exploratory local sources; this is runtime/parity evidence and a local screen, not league qualification.'}
    ...
    def run(case):
        ...
        subject = POLICY if case['name'] == 'baseline' else RAW / 'explicit-sustain/policy.bas'
        roster = [subject if i == slot else reference for i in range(10)]
        binary = RAW / 'bin/episode-v2'
