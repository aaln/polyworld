"""Frozen eight-cell guarded siege comparison against the deployed Druid baseline."""
...
ROOT=base.ROOT
STUDY=ROOT.parent/'polyworld/tmp/gota-richard174-source-20260923'
base.STUDY=h.STUDY=panel.STUDY=audit.STUDY=STUDY
h.CYCLE='interactive-richard174-source-20260923'
...
RULE='400 fresh games,50/source/context: red lead,blue lead,red late,blue late; ordinals0/3. Zero10source/VM/full-replay/XP/integer failures;>=10percent pooled mean score gain,eachcontext>=95percentcontrol,positive lower95percent context-stratified independent whole-game bootstrap gain. Stable game/principal champions. One frozen combined source; no prior hosted controls or posthoc class filtering.'
...
def upload(c):
    data=source('guarded-siege').read_bytes();out=STUDY/'uploads/guarded-siege'
    meta={'name':'cairn-orchid-d8e1','content_hash':h.sha(data),'size_bytes':len(data),'player_id':h.PLAYER,'attributes':{},'tags':{'game':'gods_of_the_arena'}}
...
def prepare():
    ...
    assert h.sha(source('guarded-siege').read_bytes())==h.read(STUDY/'guarded-siege/manifest.json')['source_sha256']
    ...
    with h.research.lock(h.CAMPAIGN/'runner.lock',blocking=False),h.client() as c:
        ...
        versions={'baseline':BASELINE,'guarded-siege':upload(c)}
        ...
        for label,version in versions.items():
            for cell,side,ordinal in [('red-lead',0,0),('blue-lead',1,0),('red-late',0,3),('blue-late',1,3)]:
                ...
                body={'idempotency_key':f'gota-cairn-d8e1-{label}-{cell}-{digest[:10]}','target':{'coworld_id':h.GAME,'variant_id':'competition'},'game_config_overrides':cfg(game),'num_episodes':50,'roster':[{'slot':i,'player':{'policy_ref':v}} for i,v in enumerate(roster)],'notes':'Controlled policy evaluation.'}
        ...
        plan={'stage':'trial','games':400,'engine_commit':h.COMMIT,'game_version':h.VERSION,'cycle':h.CYCLE,'arms':arms,'rule':RULE,'budget':'2320/11600 reserved before;400new in8x50, separate cycle,maximum3active. User authorized10000additional games September23UTC.','source_evidence':'docs/opponents/richard-v174/source-audit-20260923','source_freeze':'Guarded siege r2, generated from current Druid IR; coordinated pressure and retaliation; fresh controls; natural draft.'}
