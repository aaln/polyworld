...
ROOT=b.ROOT;STUDY=ROOT.parent/'polyworld/tmp/gota-richard174-source-20260923';OUT=ROOT/'examples/gods_of_the_arena/players/ir/forks/richard17420260923-local'
...
def main():
 ...
 src=STUDY/'guarded-siege';pair=OUT/'guarded-siege';shutil.copytree(src,pair,ignore=shutil.ignore_patterns('__pycache__'))
 ev=pair/'evidence'
 ...
 for name in ['local-summary.json','native-plan.json','native-result.json','runtime-provenance.json','conversion-proof.json','prospective-experiment.md','skill-difference.json','baseline-practice.json','preflight.json']:
  shutil.copy2(STUDY/name,ev/name)
 for label in ['practice','recovery','openings','buyback','portals','scenarios']:shutil.copy2(STUDY/f'guarded-siege-{label}.json',ev/f'{label}.json')
 ...
 write(ev/'preserved-local-revisions.json',{'raw_root':str(STUDY),'artifacts':{str(p.relative_to(STUDY)):sha(p) for p in (STUDY/'local-revisions').rglob('*') if p.is_file() and '__pycache__' not in p.parts},'reason':'r1 used fractional division on oversized raw attack range; local tests exposed BASIC range error. r2 uses integer-scaled comparison. No hosted games used r1.'})
 write(ev/'practice-provenance.json',{'source_sha256':sha(ROOT/'games/gods_of_the_arena/instruments/richard17420260923/practice.nim'),'binary_sha256':sha(STUDY/'practice'),'effect_source_sha256':sha(ROOT/'games/gods_of_the_arena/instruments/richard17420260923/source_probe.nim'),'effect_binary_sha256':sha(STUDY/'effect-probe')})
 b.configure();ir=b.ir;p=read(src/'policy.ir.json');initial=ir.digest(p)
 p['belief']['claims']['RecoveryMechanism'].update(status='supported',claim='The inherited Druid recovery component is byte-for-byte unchanged. Its92 actual-tick recovery cases pass on this complete new source; the previous Druid-only hosted result belongs to the parent pair. This coordinated siege source changes non-Druid target selection, so prior whole-policy non-Druid equivalence is not claimed.',evidence=[{'artifact':'evidence/recovery.json'},{'artifact':'evidence/parent-reviewed.ir.json'}])
 p['belief']['claims']['RichardSourceTransfer'].update(status='supported',claim='Richard source identity and ordering are verified over82818commands in4Warlock games. Our broader retrospective diagnosis finds109 guarded tower opportunities. New target selection passes80 actual-host scenes on four classes and both colors, together with582 inherited checks and16 complete native matches. Source identity, runtime and intended guarded choices are supported; competitive value of the coordinated transfer remains unvalidated.',evidence=[{'artifact':'evidence/opponent/richard_v174.source.ir.json'},{'artifact':'evidence/own-diagnosis.json'},{'artifact':'evidence/practice.json'},{'artifact':'evidence/local-summary.json'}])
