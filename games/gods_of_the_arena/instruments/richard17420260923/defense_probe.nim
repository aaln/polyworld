          ...
          if getEnv("AUDIT_KIND") == "ours":
            ...
            if ov("active")==1:
              ...
              for action in actual:
                if action.kind==ActionAttackMove and action.first==ov("homeX") and action.second==ov("homeY") and ov("retreat")==0 and ov("homeThreat")>0:
                  counts.inc("home_navigation_submitted")
                  let selfDx=mapCoordinate(hero.position.x,hero.team)-ov("homeX")
                  let selfDy=mapCoordinate(hero.position.z,hero.team)-ov("homeY")
                  var closer=0
                  var exposedGod=false
                  for oi in 0..<game.run.world.worldObjectCount(hero.id):
                    var obj:WorldObject
                    if game.run.world.worldObjectAt(hero.id,oi,obj) and obj.team==hero.team and obj.hp>0:
                      if obj.kind==1 and obj.alive:exposedGod=true
                      if obj.kind==2 and obj.id!=hero.id:
                        let dx=mapCoordinate(obj.position.x,hero.team)-ov("homeX")
                        let dy=mapCoordinate(obj.position.z,hero.team)-ov("homeY")
                        if dx*dx+dy*dy<selfDx*selfDx+selfDy*selfDy:inc closer
                  if closer>0:counts.inc("home_navigation_with_closer_ally")
                  if not exposedGod:counts.inc("home_navigation_god_not_exposed")
