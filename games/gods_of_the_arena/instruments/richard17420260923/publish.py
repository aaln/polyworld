...
OUT=ROOT/'examples/gods_of_the_arena/players/ir/forks/richard17420260923-hosted'
def main():
 report=read(STUDY/'trial/report.json');effects=read(STUDY/'trial/effects-summary.json')
 assert report['complete'] and report['games']==400 and len(effects['rows'])==32
 assert not OUT.exists(),'Preserve sealed results'
 prior=local.OUT/'guarded-siege';pair=OUT/'guarded-siege';shutil.copytree(prior,pair,ignore=shutil.ignore_patterns('__pycache__'))
 ...
 p['belief']['claims']['CompetitiveGain'].update(status='supported' if report['deployment_qualified'] else 'contradicted',claim=f"The frozen400-game qualification rule was met: {report['deployment_qualified']}. Pooled mean score change {entry['aggregate_gain_percent']:.3f}%,95%gain interval {entry['gain_ci95_percent']},context changes {entry['per_context_gain_percent']}. This result applies to one complete coordinated source and four fixed-roster contexts. Rejection is failure to qualify, not proof of population harm. It establishes neither a universal ranking nor single-component causality.",evidence=[{'artifact':'evidence/trial-report.json'}])
 totals={k:sum(r['counts'].get(k,0) for r in effects['rows'] if r['name']=='guarded-siege') for k in ['selected_covered_tower','submitted_attack_covered_tower','selected_siege_attacker','submitted_attack_siege_attacker']}
 p['belief']['claims']['RichardSourceTransfer']['claim'] += f" Hosted mechanism audit reconstructs all commands and state hashes in32prospectively selected games; candidate totals {totals}. Selections and submitted attacks do not establish landed damage or causal gain. Competitive qualification is recorded separately."
