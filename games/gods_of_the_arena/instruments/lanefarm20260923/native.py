...
POLICY = ROOT / 'examples/gods_of_the_arena/players/ir/forks/controltactics20260923-hosted/control-tactics/policy.bas'
...
def main():
    ...
    cases = [{'name': name, 'side': side, 'ordinal': ordinal, 'seed': seed} for name in ['baseline', 'lane-occupancy'] for side in range(2) for ordinal in [2, 4] for seed in [9230630, 9230631]]
    plan = {'cases': cases, 'own_source_sha256': sha(POLICY), 'candidate_source_sha256': sha(RAW / 'lane-occupancy/policy.bas'), 'upstream_source_sha256': sha(reference), 'scope': 'Sixteen observations: eight preserved control-tactics native games and eight new lane-occupancy games, one subject and nine current reference VMs. Matched seed/roster/slot/config with nine responsive current reference VMs; runtime evidence, not league qualification.'}
    ...
    def run(case):
        ...
        if case['name'] == 'baseline':
            out = ROOT.parent / 'polyworld/tmp/gota-control-tactics61-20260923/native/control-tactics' / out.name
            assert (out / 'result.json').exists()
        out.mkdir(parents=True, exist_ok=True)
        if (out / 'result.json').exists():
            return dict(json.loads((out / 'result.json').read_text()), **case)
        subject = POLICY if case['name'] == 'baseline' else RAW / 'lane-occupancy/policy.bas'
    ...
    baseline, candidate = rows[:8], rows[8:]
    for a, b in zip(baseline, candidate):
        assert (a['side'], a['ordinal'], a['seed']) == (b['side'], b['ordinal'], b['seed'])
        assert a['subject']['class'] == b['subject']['class'] and a['valid'] and b['valid']
    write(RAW / 'native-comparison.json', {'passed': True, 'pairs': 8, 'new_native_games': 8,
        'baseline_total': sum(r['subject']['score'] for r in baseline),
        'candidate_total': sum(r['subject']['score'] for r in candidate),
        'score_deltas': [b['subject']['score'] - a['subject']['score'] for a,b in zip(baseline,candidate)],
        'scope': 'Runtime validity passes, not a score gate; current reference opponents differ from the frozen hosted field. All comparative native outcomes retained.'})
