...
def decode(folder):
    ...
    p=subprocess.run([str(RAW/'bin/sharing'),'--replay',str(folder/'replay.bin')],capture_output=True,text=True,timeout=900)
    (dest/'telemetry-stderr.log').write_text(p.stderr)
...
def audit_pair(r,plan):
    ...
    for label in ['baseline','candidate']:
        ...
        a,t,s,v,res=[read((RAW/'sharing'/folder.name/n) if n == 'telemetry.json' else folder/n) for n in ['audit.json','telemetry.json','spec.json','player-status.json','results.json']]
...
def metrics(rows,arm):
    ...
    for r in rows:
        own=r[arm]; folder=RAW/'artifacts'/own['episode'];t=read(RAW/'sharing'/folder.name/'telemetry.json')
        ...
        for count in range(1,6):
            d[f'creep_xp_shared_with_{count}_recipients']=hero['creep_xp_by_recipient_count'][count]
            d[f'creep_receipts_with_{count}_recipients']=hero['creep_receipts_by_recipient_count'][count]
...
def main():
    ...
    for arm in ['baseline','candidate']:
        ...
        for r in rows:
            own=r[arm];sha=read(RAW/'sharing'/own['episode']/'telemetry.json')['canonical_commands_sha1']
            groups[sha].append(own['episode'])
        result['streams'][arm]=dict(unique=len(groups),duplicates=[v for v in groups.values() if len(v)>1])
    result['pilot_advance'] = overall['mean_delta'] > 0 and overall['delta95'][0] >= 0
    result['deployment_qualified']=False
    result['scope']='Directional score-only pilot on80reusedbaseline seeds acrossfourfixedcontexts. Reused discovery cohort, not independent confirmation. All80candidate games and80capturedbaselinegames audited. No established verdictsamplefloor; no deploymentqualification. Hero/color/seat andXPsource/time diagnostics are exploratory. Other player comparisons are not causal.'
