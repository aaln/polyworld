"""Frozen six-cell coaching comparison: fresh controls, actual production bytes."""
...
h.CYCLE='interactive-field-sustain-20260923'
...
def upload(c):
    data=source('field-sustain').read_bytes();out=STUDY/'uploads/field-sustain'
    meta={'name':'aaron-gota-sustain0923-field-sustain','content_hash':h.sha(data),'size_bytes':len(data),'player_id':h.PLAYER,'attributes':{},'tags':{'game':'gods_of_the_arena','game_version':h.VERSION,'engine_commit':h.COMMIT,'validation':'68sustain+100opening+180buyback+84portal+126broad checks;12native full games;portable IR equality. Inert coaching candidate; hosted score unvalidated.'}}
...
def prepare():
    ...
    assert h.sha(source('field-sustain').read_bytes())==h.read(STUDY/'field-sustain/manifest.json')['source_sha256']
    ...
    with h.research.lock(h.CAMPAIGN/'runner.lock',blocking=False),h.client() as c:
        ...
        versions={'baseline':BASELINE,'field-sustain':upload(c)}
        template=h.read(STUDY/'roster-template-v2.json');arms=[]
        for label,version in versions.items():
            for cell,side,ordinal in [('blue-lead',1,0),('red-late',0,3),('blue-late',1,3)]:
                ...
                body={'idempotency_key':f'gota-sustain0923-{label}-{cell}-{digest[:10]}','target':{'coworld_id':h.GAME,'variant_id':'competition'},'game_config_overrides':cfg(game),'num_episodes':40,'roster':[{'slot':i,'player':{'policy_ref':v}} for i,v in enumerate(roster)],'notes':RULE}
        ...
        plan={'stage':'trial','games':240,'engine_commit':h.COMMIT,'game_version':h.VERSION,'cycle':h.CYCLE,'arms':arms,'rule':RULE,'budget':'1360/1600reserved before;240new,6x40,max3active; normalSep23UTC cap.','coaching_session':'2026-09-23t02-52-57-098ze03810','source_freeze':'Candidate r2 and deployed blue-center controls; all components coordinated, actual draft unchanged.'}
        h.freeze(out/'plan.json',plan)
        print(json.dumps({'prepared':True,'games':240,'new_requests':0}),flush=True)
def run():
