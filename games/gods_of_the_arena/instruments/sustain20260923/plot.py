...
import numpy as np
ROOT=Path(__file__).resolve().parents[4];STUDY=ROOT.parent/'polyworld/tmp/gota-field-sustain-20260923'
...
for i,n in enumerate(names):
 y=[cells[n,s]['means']['score'] for s in ['blue-lead','red-late','blue-late']];ci=[cells[n,s]['score_ci95'] for s in ['blue-lead','red-late','blue-late']]
 x=np.array([0,1,2])+(i-(len(names)-1)/2)*width
 bars=ax.bar(x,y,width,label=n,color=colors[i]);ax.errorbar(x,y,yerr=[[y[j]-ci[j][0] for j in range(3)],[ci[j][1]-y[j] for j in range(3)]],fmt='none',color='#293644',capsize=3)
 ax.bar_label(bars,fmt='%.0f',padding=3,fontsize=9)
ax.set_xticks([0,1,2],['Blue lead','Red late','Blue late']);ax.set_ylabel('Mean individual points');ax.set_title('Score by context · 40 games per bar');ax.legend(fontsize=8);ax.spines[['top','right']].set_visible(False)
components=['hero_xp','creep_xp','building_xp','god_xp','time','clamp/round'];palette=['#9e4937','#438363','#a89246','#786794','#687e97','#a8a8a8']
labels=[];rows=[]
for n in names[1:]:
 for side in ['blue-lead','red-late','blue-late']:
...
fig.suptitle('GOTA field sustain · '+a.stage+' · 2026.9.22.3',fontsize=15)
fig.text(.06,.02,'Left: 95% whole-game bootstrap intervals. Right: exact mean score decomposition, including zero-clamp/rounding residual.\nFixed mixed rosters; later draft includes two fixed reference teammates. Effects are descriptive. One preselected candidate; 240 fresh held-out games.',fontsize=9,color='#52606d')
