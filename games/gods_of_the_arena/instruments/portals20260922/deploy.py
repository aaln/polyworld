...
import httpx
import jsonschema
from study import h, ROOT, STUDY

LEAGUE = 'league_3c60897b-25cf-4b37-9d1a-8554c1198f28'
PLAYERS = {'aaron': h.PLAYER, 'coach': 'ply_594ec24d-d7f3-4370-a000-468354ec41c9'}
PRIOR = {'aaron': '0dc85085-cb5b-4de6-8d50-e8fd043d0b5f', 'coach': '35c85505-1977-4bad-b4ea-f1473aaa79dc'}
PAIR = ROOT / 'examples/gods_of_the_arena/players/ir/forks/portal-coaching20260922-hosted'
OUT = STUDY / 'deployment'
NOTE = ('Portal recovery coaching, exact patched2026.9.22.2 bytes. '
        'Passed prospective 160-game fresh-control A/B: +11.56% mean XP score, '
        'red+27.46%, blue+0.60%; all source/VM/replay/XP/integer-score audits pass. '
        'Keep-to-home-classified portals fall107 to1. Outscored khors:v114 '
        '45/80games and on both color means, but confidence intervals includezero. '
        'Fixed first-pick mixed roster; no robust khors-superiority or #1 claim. '
        'Prior explicit authorization to deploy latest validated policy to Aaron and Coach.')
...
def upload_coach(c, source, schema):
    out = OUT / 'coach'
    meta = {'name': 'aaron-gota-portals0922-coach', 'content_hash': h.sha(source),
...
def main():
    with h.research.lock(h.CAMPAIGN / 'league-deployment.lock', blocking=False), h.client() as c:
        ...
        result = h.read(STUDY / 'hosted-khors114/result.json')
        ...
        if not (OUT / 'before.json').exists():
            ...
            h.freeze(OUT / 'decision.json', {'at':h.research.now(),'source_sha256':h.sha(source),
                ...
                'engine':h.COMMIT,'game_version':h.VERSION,'evidence':'160-game fresh-control portal A/B against khors:v114; full report preserved',
                'score':result['score'],'control_score':result['control_score'],'uplift_percent':(result['score']/result['control_score']-1)*100,
                'rollback_versions':PRIOR,'rollback_source_sha256':'7631fa32fb7ef6725074ad6778f4f94fe1e18ac5018bece1eed40e32cbef9bb8',
                'rollback_source':'examples/gods_of_the_arena/players/ir/forks/balance-draft20260922/policy.bas',
                'limits':'Fixed first-pick roster; wide uncertainty and one duplicate candidate-blue stream. Khors confident superiority remains unqualified; no #1 guarantee. Fresh controls, fully audited.'})
        versions = {'aaron':h.read(STUDY / 'upload/version.json'),
                    'coach':upload_coach(c,source,schema)}
        log = ROOT / 'games/gods_of_the_arena/players/portals20260922/VERSION_LOG.md'
        text = log.read_text()
        ...
        log.write_text(log.read_text() + f"\n- {h.research.now()}: Both players verified active, competing champions on `{h.sha(source)}`. Receipt: `tmp/gota-portals-20260922/deployment/verified.json`.\n")
