...
ENGINE = ROOT.parent / 'polyworld-gota-neutral62-20260923'
POLICY = ROOT / 'examples/gods_of_the_arena/players/ir/forks/controltactics20260923-hosted/control-tactics/policy.bas'
SOURCES = {'baseline': POLICY, 'lane-only': ROOT / 'examples/gods_of_the_arena/players/ir/forks/lanefarm20260923-hosted/lane-occupancy/policy.bas', 'lane-neutral': RAW / 'lane-neutral/policy.bas'}
...
def main():
    ...
    cases = [{'name': name, 'side': side, 'ordinal': ordinal, 'seed': seed} for name in SOURCES for side in range(2) for ordinal in [0, 3] for seed in [9230620]]
    plan = {'cases': cases, 'source_hashes': {k:sha(v) for k,v in SOURCES.items()}, 'upstream_source_sha256': sha(reference), 'scope': 'Twelve fresh replay62 native games; baseline, lane-only, lane-plus-neutral; one subject, nine responsive current reference VMs; two sides and two seats, one matched seed. Runtime/mechanism screen only.'}
