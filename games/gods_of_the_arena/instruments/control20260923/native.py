...
def main():
    ...
    cases = [{'name': name, 'side': side, 'ordinal': ordinal, 'seed': seed} for name in ['baseline', 'control-legality'] for side in range(2) for ordinal in [0, 4] for seed in [9230620]]
    plan = {'cases': cases, 'own_source_sha256': sha(POLICY), 'candidate_source_sha256': sha(RAW / 'control-legality/policy.bas'), 'upstream_source_sha256': sha(reference), 'scope': 'Eight complete games, four fixed baseline controls and four candidate games, one subject and nine current reference VMs. Matched seed/roster/slot/config with nine responsive current reference VMs; runtime evidence, not league qualification.'}
    ...
    def run(case):
        ...
        subject = POLICY if case['name'] == 'baseline' else RAW / 'control-legality/policy.bas'
    ...
    write(RAW / 'native-result.json', {'passed': True, 'games': 8, 'rows': rows, 'scope': plan['scope'], 'new_hosted_games': 0})
