...
def main():
    ...
    assert not (RAW / 'control-legality-practice.json').exists(), 'Preserve prior fixture captures before rerunning'
    ...
    def build(entry):
        name, source = entry
        dest = ENGINE / 'examples/gods_of_the_arena/tools' / ('control61_' + name.replace('-', '_') + '.nim')
    ...
    for label, policy in [('baseline', PARENT / 'policy.bas'), ('control-legality', RAW / 'control-legality/policy.bas')]:
