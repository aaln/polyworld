...
sys.path.insert(0,str(ROOT));sys.path.insert(0,str(CLEAN))
...
STUDY=ROOT/'tmp/gota-ir/richard-counter-20260920';PLAYER='ply_630a768f-d623-44b2-80fa-36968d6fa75a';RIVAL='7c370daf-3c5f-42f8-870b-54b79c495a44';PARENT='4cdbbf36-3d70-4ea3-8aed-c92ee0e024be';BINARY=ROOT.parent/'gota-research-20260916/r5/fast/audit-hosted'
...
def upload(name):
 ...
 meta={'name':'aaron-gota-ir-richard135-'+name.replace('_','-')+'-0920','content_hash':digest(source),'size_bytes':len(source),'player_id':PLAYER,'attributes':{},'tags':{'game':'gods_of_the_arena','game_version':'2026.9.16.5','change':name,'semantic_ir_sha256':digest(policy),'validation':'unvalidated experimental fork; not league champion'}}
 ...
 with client() as c:
  ...
  log=ROOT/'games/gods_of_the_arena/players/richard135-counter/VERSION_LOG.md';text=log.read_text() if log.exists() else '# Richard135 counter-policy forks\n'
...
def arm(name,color,n,stage):
 ...
 reference=read(STUDY/'reference-episode.json');config={k:v for k,v in reference['game_config'].items() if k not in ('seed','players','tokens')};roster=[policy if slot in own else RIVAL for slot in range(10)]
 plan={'candidate':name,'stage':stage,'color':color,'n':n,'own_slots':own,'policy_version':policy,'rival_version':RIVAL,'roster':roster,'config':config,'game_version':game['version'],'target':{'coworld_id':game['id'],'variant_id':'competition'}}
 freeze(folder/'plan.json',plan)
 pin_auditor(folder)
 body={'idempotency_key':'r135-counter-'+digest(plan)[:24],'target':plan['target'],'game_config_overrides':config,'num_episodes':n,'roster':[{'slot':i,'player':{'policy_ref':v}} for i,v in enumerate(roster)],'notes':f'Richard135 IR-guided fork {name}, {color}, {stage}. All10seats pinned. {n} generated-seed cohort; not seed-matched. No promotion.'}
 with client() as c:
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('command',choices=['upload','arm','screen']);p.add_argument('name');p.add_argument('--color',choices=['red','blue']);p.add_argument('--n',type=int,default=4);p.add_argument('--stage',default='screen');p.add_argument('--rival');a=p.parse_args();RIVAL=a.rival or RIVAL
