...
proc decision(policy: string, team: Team, class: HeroClass,
  ...
  game.world.tick = 6
  if scenario == Kiting:
    # Prime real persistent policy state before observing a fresh landed hit.
    game.runBotDecisions()
    game.world.tick += 1
    hero.attackObjectId = enemy.id
    hero.attacksLanded += 1
    hero.swingTicks = 0
    game.recorder.data.actions.setLen(0)
  game.runBotDecisions()
...
block:
  var checked = 0
  for policy in [getEnv("WEEK_POLICY")]:
      ...
      for scenario in Scenario:
        for boundary in [false, true]:
          ...
          doAssert red.actions == blue.actions,
            label & "\nred: " & $red.actions & "\nblue: " & $blue.actions
          doAssert max(red.instructions, blue.instructions) <= 19000, label
          doAssert max(red.work, blue.work) <= 50000, label
          inc checked
