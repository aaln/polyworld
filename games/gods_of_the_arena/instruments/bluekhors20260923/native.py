...
def run(case):
    ...
    source=ROOT/'examples/gods_of_the_arena/players/ir/forks/core-buyback20260923/core-buyback/policy.bas' if label=='baseline' else STUDY/label/'policy.bas'
...
def main():
    cases=[(n,s,k) for n in ['baseline','blue-center'] for s in [0,1] for k in [923020,923021]]
    plan={'cases':cases,'engine':'1b70894436b7ffdcd0d421b6b32c2415c9c8bfde','reference':sha(ENGINE/'examples/gods_of_the_arena/players/base.bas'),'candidates':{n:sha(STUDY/n/'policy.bas') for n in ['blue-center']},'config':sha(STUDY/'game-config.json'),'scope':'One subject, nine responsive native reference VMs. Mechanism/runtime, not rival-policy comparison.'}
