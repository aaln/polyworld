...
def build(stage):
    ...
    for entry in candidates:
        old_blue=before[1];new_blue=by[entry['name']][1];rng=random.Random(923022);blue_gains=[]
        for _ in range(10000):
            old_mean=mean(rng.choices([r['score'] for r in old_blue['rows']],k=100));new_mean=mean(rng.choices([r['score'] for r in new_blue['rows']],k=100));blue_gains.append((new_mean/old_mean-1)*100)
        blue_gains.sort();gap=next(o for o in new_blue['opponents'] if o['label']=='khors:v114')['own_minus_rival']
        entry['blue_gain_ci95_percent']=[blue_gains[250],blue_gains[9750]];entry['blue_mean_khors_gap']=gap
        entry['aggregate_gate_passed']=entry['score_gate_passed'];entry['score_gate_passed']=entry['score_gate_passed'] and blue_gains[250]>0 and gap>0
