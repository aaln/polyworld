...
PRIOR={'aaron':'30a0e469-8c2f-450c-b3bc-4a687a6c74e3','coach':'2013aad3-a754-4e31-8cea-727aa6c3a6b1'}
placement.PRIOR=PRIOR

def upload_coach(c,source,schema,note):
    out=OUT/'coach';meta={'name':'aaron-gota-bluecenter0923-coach','content_hash':h.sha(source),'size_bytes':len(source),'player_id':PLAYERS['coach'],'attributes':{},'tags':{'game':'gods_of_the_arena','game_version':h.VERSION,'engine_commit':h.COMMIT,'validation':note}}
...
def main():
    ...
    selected=confirmation['selected'];assert selected=='blue-center' and confirmation['games']==400
    result=confirmation['candidates'][0];assert result['score_gate_passed']
    pair=ROOT/'examples/gods_of_the_arena/players/ir/forks/blue-center20260923-hosted'/selected
    ...
    note=f"Blue lead ranged central route plus immediate critical keep recovery. One preselected source,400fresh held-out games,100/source/color, fixed first team seat: gain{result['aggregate_gain_percent']:.3f}%, colors{result['per_color_gain_percent']},95% interval{result['gain_ci95_percent']}. All10source/VM/replay/XP/integer audits. ExactRichard174 andkhors114; no universal ranking guarantee. Prior user authorization for both players."
    placement.NOTE=note
