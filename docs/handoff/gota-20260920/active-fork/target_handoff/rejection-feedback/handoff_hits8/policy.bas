' Generated from autoresearch_20260920_handoff_hits8; gota-semantic-policy/1; gota-basic/1
...
if objectCount() > defMotionLimit then
  motionActive = 0
  if bestId <> 0 and selfClass <> 9 and (selfLevel >= 2 or (8 > 0 and selfAttacksLanded >= 8) or (0 = 1 and defActive = 0)) then
    if selfClass = 1 or selfClass = 6 or (1 = 1 and (selfClass = 0 or selfClass = 4 or selfClass = 5)) then
      if worldTick = denseLastTick + 1 and denseLastTick > 0 and selfAttacksLanded > denseLastHits and selfAttackCooldown > 0 then
        ...
        if mTracked and mKind <> 2 and bestId <> selfTarget then
