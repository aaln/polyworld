' Generated from autoresearch_transit_anchor_v1; gota-semantic-policy/1; gota-basic/1
...
if selfTeam = 0 then
  ...
  if defActive then
    ...
    if selfClass = 7 or selfClass = 8 then
    aaReady = 0
    aaCommitted = 0
    aaDx = defFrontX - selfX
    aaDy = defFrontY - selfY
    aaDistance = aaDx * aaDx + aaDy * aaDy
    if defFront <> 0 and aaDistance <= 22 * 22 then
      aaI = 0
      while aaI < observedObjectCount and aaI < 64
        ...
        if aaKind = 3 then
          ...
          if aaKind = 2 and objectTeam(aaI) = selfTeam and objectHp(aaI) >= 120 and objectAlive(aaI) and objectId(aaI) <> selfId then
            ...
            if aaDx * aaDx + aaDy * aaDy <= 144 then
              aaReady = aaReady + 1
              if objectTarget(aaI) = defFront and (0 = 0 or objectClass(aaI) = 5) then
                aaCommitted = 1
              end if
            end if
          end if
        ...
        aaI = aaI + 1
      wend
      if aaCommitted and selfHp * 100 >= selfMaxHp * 25 and (1 = 0 or bestId = 0)  and (1 = 0 or defAnchor <> 0) then
        bestId = defFront
        bestDistance = aaDistance
      end if
      if 0 and selfClass = 0 and bestId = defFront and aaReady = 0 and aaDistance > 16 then
        bestId = 0
      end if
    end if
    end if
