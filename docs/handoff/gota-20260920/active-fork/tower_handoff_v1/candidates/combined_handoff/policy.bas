' Generated from autoresearch_tower_combined_handoff_v1; gota-semantic-policy/1; gota-basic/1
    ...
    if selfClass = 9 or selfClass = 7 then
      ...
      while index < observedObjectCount
        ...
        if objectTeam(index) <> selfTeam then
          ...
          if bestId = 0 or bestDistance > 12 * 12 * 1 then
...
if selfTeam = 0 then
  ...
  if towerReleaseUntil > 0 and worldTick >= towerReleaseUntil then
    towerReleaseUntil = 0
    towerReleaseReady = worldTick + 48
  end if
  if worldTick < towerReleaseUntil or (worldTick >= towerReleaseReady and bestId <> 0 and selfHp * 100 <= selfMaxHp * 50) then
    mThreat = bestId
    ...
    while mIndex < objectCount() and mIndex < 64 and mFound = 0
      if objectId(mIndex) = mThreat then
        if objectKind(mIndex) = 4 and objectTeam(mIndex) <> selfTeam and objectHp(mIndex) > 0 and objectTarget(mIndex) = selfId then
          if worldTick < towerReleaseUntil or (objectHp(mIndex) > 150 and objectHp(mIndex) > selfAttackDamage * 2) then
            mFound = 1
      ...
      if worldTick >= towerReleaseUntil then
        ...
        if mFound then
          ...
          if mFound then
            ...
            towerReleaseUntil = worldTick + 144
