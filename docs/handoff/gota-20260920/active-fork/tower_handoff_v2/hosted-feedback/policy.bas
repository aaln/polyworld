' Generated from autoresearch_tower_transit_handoff_v2; gota-semantic-policy/1; gota-basic/1
...
if selfTeam = 0 then
  ...
  if defFront <> 0 then
    ...
    if defAnchor <> 0 and (pairedRush or middleRush or defCount >= defGroupSize or (worldTick < defUntil and defFrontD <= defContinueHome * defContinueHome)) then
