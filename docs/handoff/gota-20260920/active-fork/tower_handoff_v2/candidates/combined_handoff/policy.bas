' Generated from autoresearch_tower_combined_handoff_v2; gota-semantic-policy/1; gota-basic/1
...
if selfTeam = 0 then
  ...
  defI = 0
  while defI < observedObjectCount and defI < 2
  ...
  defFrontD = 100 * 100 + 1
  defI = 0
  while defI < observedObjectCount and defI < 64
  ...
  if defFront <> 0 then
    ...
    while defI < observedObjectCount and defI < 64
      ...
      defI = defI + 1
    wend
    if defAnchor <> 0 and (defCount >= defGroupSize or (worldTick < defUntil and defFrontD <= defContinueHome * defContinueHome)) then
  ...
  if defDx * defDx + defDy * defDy > 28 * 28 then
    defActive = 0
    defUntil = 0
  end if
  ...
  if defActive then
    ...
    defI = 0
    while defI < observedObjectCount
      defKind = objectKind(defI)
    ...
    if selfClass = 9 or selfClass = 7 then
      ...
      defMates = 12 * 12 * 1
      index = 0
      while index < observedObjectCount
        id = objectId(index)
  ...
  while defI < observedObjectCount and defI < 2
    ...
    defI = defI + 1
  wend
  responseDx = selfX - defHomeX
  responseDy = selfY - defHomeY
  responseOwnD = responseDx * responseDx + responseDy * responseDy
  responseRank = 0
  responseIndex = 0
  while responseIndex < observedObjectCount and responseIndex < 64 and responseRank < 3
    ...
    if responseKind = 3 then
      ...
      if responseKind = 2 and objectTeam(responseIndex) = selfTeam and objectAlive(responseIndex) and objectHp(responseIndex) > 0 then
        responseDx = objectX(responseIndex) - defHomeX
        responseDy = objectY(responseIndex) - defHomeY
  ...
  wend
  if responseMode and responseRank >= 3 then
    responseUntil = 0
    responseMode = 0
    defUntil = 0
  end if
  ...
  if defFront <> 0 then
    ...
    responseEligible = 0
    if defAnchor <> 0 and defFrontD <= 60 * 60 and defCount >= 2 then
      if responseRank < 3 then
        responseEligible = 1
      end if
    end if
    if defAnchor <> 0 and (responseEligible or pairedRush or middleRush or defCount >= defGroupSize or (worldTick < defUntil and defFrontD <= defContinueHome * defContinueHome)) then
      perimeterAnchor = defAnchor
      perimeterX = defAnchorX
      perimeterY = defAnchorY
      defUntil = worldTick + defHoldTicks
      if responseEligible then
        responseUntil = worldTick + 240
        responseMode = 1
      end if
  ...
  if selfTeam = 1 and defCount <= 1 and defFront <> 0 and defFrontD <= 24 * 24 then
    if defAnchor = defFrontTarget and (defFrontTarget = backdoorGod or defFrontTarget = backdoorGuardA or defFrontTarget = backdoorGuardB) then
      ...
      backdoorHelpers = 0
      backdoorI = 0
      while backdoorI < observedObjectCount and backdoorI < 64
  ...
  if selfTeam = 1 then
    defDx = selfX - defHomeX
    defDy = selfY - defHomeY
    if defDx * defDx + defDy * defDy > 28 * 28 and worldTick >= responseUntil then
      defActive = 0
      defUntil = 0
      backdoorUntil = 0
      backdoorActive = 0
...
if selfTeam = 0 and (selfClass = 7 or selfClass = 8) then
  if worldTick <= 1800 and transitDone = 0 then
    ...
    if transitDx * transitDx + transitDy * transitDy <= 6 * 6 then
      transitDone = 1
    else
      if transitObservedDefense = 0 then
        transitActive = 1
      end if
    end if
  end if
...
' @rule R_tower_handoff
if selfTeam = 0 then
  responseEligible = 0
  if worldTick <> responseOwnD + 1 then
    responseUntil = 0
    responseMode = 0
  end if
  responseOwnD = worldTick
  if responseUntil > 0 and worldTick >= responseUntil then
    responseUntil = 0
    responseMode = worldTick + 48
  end if
  if worldTick < responseUntil or (worldTick >= responseMode and bestId <> 0 and selfHp * 100 <= selfMaxHp * 50) then
    mThreat = bestId
    if worldTick < responseUntil then
      mThreat = responseKind
    end if
    mFound = 0
    mIndex = 0
    while mIndex < objectCount() and mIndex < 64 and mFound = 0
      if objectId(mIndex) = mThreat then
        if objectKind(mIndex) = 4 and objectTeam(mIndex) <> selfTeam and objectHp(mIndex) > 0 and objectTarget(mIndex) = selfId then
          if worldTick < responseUntil or (objectHp(mIndex) > 150 and objectHp(mIndex) > selfAttackDamage * 2) then
            mFound = 1
    ...
    if mFound = 0 then
      ...
      if worldTick >= responseUntil then
        ...
        if mFound then
          ...
          wend
          if mFound then
            responseKind = mThreat
            responseUntil = worldTick + 144
            responseD = responseD + 1
          else
            responseMode = worldTick + 48
          end if
        end if
      ...
      if worldTick < responseUntil then
        if walkTo(responseDx, responseDy) then
          responseEligible = 1
          responseIndex = responseIndex + 1
        else
          responseUntil = 0
