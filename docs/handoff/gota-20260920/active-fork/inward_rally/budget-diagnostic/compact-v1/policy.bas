' Generated from diagnostic_compact_wave_v1; gota-semantic-policy/1; gota-basic/1
    ...
    while defI < objectCount()
      ...
      if (defKind = 2 or defKind = 3) and objectTeam(defI) <> selfTeam and objectAlive(defI) and objectHp(defI) > 0 then
        ...
        if defD <= 10 * 10 and (defGroupD <= 400 or defD <= 9) then
          ...
          if (0 = 1 and defKind = 3) or (0 = 0 and defKind = 2) then
...
if objectCount() > defMotionLimit then
  motionActive = 0
  if bestId <> 0 and selfClass <> 9 and (selfLevel >= 2 or (0 = 1 and defActive = 0)) then
    if selfClass = 1 or selfClass = 6 or (1 = 1 and (selfClass = 0 or selfClass = 4 or selfClass = 5)) then
      ...
      while index < objectCount()
        if objectTeam(index) = selfTeam and objectHp(index) > 0 and objectAlive(index) then
          ...
          if objectKind(index) = 3 then
            ...
            dy = objectY(index) - selfY
            distance = dx * dx + dy * dy
            if id = escortId or (retained = 0 and (distance < chosenDistance or (distance = chosenDistance and id < chosenId))) then
              if id = escortId then
                retained = 1
              end if
              chosenId = id
