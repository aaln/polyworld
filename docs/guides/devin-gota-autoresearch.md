...
`verify` checks the archived Jordan fork's exact Python/JSON/BASIC/compiler bundle, the saved Richard coaching pairs and opponent-model tests without network access. It does not prove competitive superiority. The old `install.py`/LaunchAgent/Codex supervisor are Mac orchestration archives; Devin is the researcher and should not launch another Codex researcher.
