...
## Cycle 11 — 2026-09-21T18:23:37Z (engine 2026.9.16.5, no drift)
Ranks: relh #1 ce5bfbfa v159 (1916.1), Richard #2 7c370daf (1795.5), Coach #3 2bb94c84 (1703.8), Aaron #4 61148477 (1656.2), macromackie #5, Alex #6 a30542cb (1624.3), Jordan #7 207ffaf9 (1615.4), NanosaurusX #8.
H2H window: Aaron 13W/13L/1D/3inv, Coach 16W/10L/4D/2inv; Aaron/Richard red 0-2, Coach/Richard red 0-1; relh trigger sustained. Ownership unchanged (Aaron incumbent, Coach challenger, a-aron retired). Hosted: 0 active, no new spend (160/1600 today); league-watch episodes.json 288 entries (264 completed/16 pending/8 running) are league observations, not our purchases.
H-FARM-01 cadence trace (cycle-10 red-54 replay -> replay_command_stream_v1, 73,861 commands): our slot 0 issues 4,661 walkTo vs 1,021 attackTarget, zero attacks before ~tick 900; all five red heroes walkTo on 77-82% of ticks. Richard issues attackTarget from ~tick 450 every tick. Engine acquireRadius()=0 for a hero with hasMoveTarget and not attackMoving -> escorting with walkTo never auto-acquires.
H-ESCORT-ATKMOVE-01 (skill.fallback route contract formation_profile_route_escort_attackmove_v1: both `moveAccepted = walkTo(moveX, moveY)` sites -> attackMove; zero new globals; frozen compiler 79 modules, compiler-exact; src bf7086ae…, ir c381fed7…; diff vs parent = 2 lines + header):
 - vs Richard v135 red 0-3 (5898 ticks, hash trajectory identical to red-def-release parent -> NO effect: attackMove reissued every tick clears attackObjectId before the acquired target can be hit), blue 0-3 (6677 ticks; parent 3-0 -> REGRESSION); vs parent red 0-2-1, blue 3-0; 0 INVALID; 3 streams/colour; max instr 18410.
 FALSIFIED: per-tick reissue of any move command resets acquisition; escort auto-acquire needs a hold (skip reissue while attackMoving toward the same target) rather than a command swap. Nothing deployed, no membership change.
Next: H-ESCORT-HOLD-01 — in the route contract, skip reissuing the move when the previous accepted command was attackMove to the same (moveX,moveY) tile (reuse existing memory slots, zero new globals), so the engine's attack-move acquisition can complete a basic hit; gate red vs Richard must beat 5898-tick collapse and blue must stay 3-0. No hosted spend until red beats parent locally.

## Cycle 12 — 2026-09-21T20:23:38Z (engine 2026.9.16.5, no drift)
Ranks: relh #1 ce5bfbfa v159 (1924.8), Richard #2 7c370daf (1777.4), Coach #3 2bb94c84 (1692.4), Aaron #4 61148477 (1666.7), Alex #5 a30542cb (1636.3), Jordan #6 207ffaf9 (1620.5), #7 1590.7, #8 1562.4. Order unchanged since cycle 09.
H2H window: Aaron/Richard red 0-2, Aaron/relh blue 0-2 red 0-1, Aaron/macromackie v4 blue 0-2, Andrew Brower red-kite:v34 red 0-2; Coach/Richard red 0-1; Coach vs Aaron red 0-6 (own-policy mirror, blue-favoured map). relh trigger sustained. Ownership unchanged (Aaron incumbent, Coach challenger, a-aron retired); account shows no XP requests by any writer since 10:34Z (latest xreq_72487884), 0 pending/submitted/running. No new spend (160/1600 today).
H-ESCORT-HOLD-01 (route contract formation_profile_route_escort_attackmove_hold_v1: `attackMove(moveX, moveY)` issued only on branch entry (worldTick <> lastJoinTick+1) or every 48 ticks (>= longest class attack period 36), so engine attack-move acquisition can swing and land; zero new globals; frozen compiler 79 modules, compiler-exact; src 1d1b5c23…, ir d995bf6d…; diff vs parent = 2x(2 lines) + header):
 - vs Richard v135 red 0-3 (4047/3988/6060 ticks, our deaths 18-19 vs parent 7, lvl 10-15 vs 21), blue 0-3 (5919 ticks, deaths 28, lvl 17 vs 31; parent 3-0 -> REGRESSION); vs parent red 0-3 (5747), blue 3-0; 0 INVALID; 3 streams/colour; max instr 14908 (acquisition path is cheaper).
 FALSIFIED (worse than cycle 11): auto-acquired creep targets pull escorting heroes off the wave into enemy heroes/towers; R2 combat control does not re-take the channel fast enough, deaths ~3x. Attack-move escort is the wrong mechanism for the farm deficit at this arbitration; the deficit must be addressed inside R2's explicit target selection (which creeps it attacks and how often it re-evaluates), not the route channel. Nothing deployed, no membership change.
Next: H-FARM-R2-01 — instrumented trace of R2's attackTarget selection on red-54 ticks 0-1500 (target kind/distance per call, acceptance, hits landed) vs Richard's target choice at the same ticks; hypothesis that R2 re-targets every tick (resetting swingTicks) rather than holding a target through its windup. Gate unchanged: red must beat the parent collapse locally and blue stay 3-0 before any hosted spend.

## Cycle 13 — 2026-09-21T22:23:37Z (engine 2026.9.16.5, no drift)
Ranks: relh #1 ce5bfbfa v159, Richard #2 7c370daf (1762.1), Coach #3 2bb94c84 (1714.9), Aaron #4 61148477 (1660.8), Jordan #5 (champion changed -> Jordan-ply_bcb80069…:v284; old 207ffaf9 evidence retained, pin fresh tests), Alex #6 a30542cb (1615.1).
H2H window: Aaron/relh blue 0-4, Aaron/Richard red 0-2, Aaron/Jordan red 0-1; Coach/relh blue 0-2 red 0-1, Coach/Richard blue 5-0 red 0-1. relh trigger sustained. Ownership unchanged (Aaron incumbent, Coach challenger, a-aron retired); account: 0 pending/submitted/running, latest request still xreq_72487884 (10:34Z), no other writer. No new spend (160/1600 today).
H-FARM-R2-01 trace (cycle-10 red-54 command stream, slot 0 vs Richard slot 0, ticks 0-1500): R2 is NOT retargeting — our slot 0 holds attackTarget 1012 for a contiguous run (ticks 906-1030+) and swaps only when the target dies. Richard's pattern is `attackTarget(x); walkTo(fixedTile)` each tick from ~450. The actual mechanism is the ROUTE channel: R4's escort issues walkTo(escortX+dx*0, escortY) every tick to a MOVING creep, so hasMoveTarget stays true and engine acquireRadius()=0 -> a hero that has no explicit R2 target never idle-acquires anything while escorting (~900 ticks with zero attacks, confirmed).
H-ESCORT-IDLE-01 (route contract formation_profile_route_escort_idle_v1: skip the per-tick walkTo once |selfX-moveX|<=1 and |selfY-moveY|<=1 so followHeroPath clears hasMoveTarget and idle acquisition (melee 2.5 tiles / ranged range) engages; reuses ax/ay scratch, zero new globals; frozen compiler 79 modules, compiler-exact; src f79b4f9a…, ir 2fe1c046…; diff vs parent = 2x(12 lines) + header):
 - vs parent red **1-2** (W 22266 ticks; our lvl 35/47/51 vs parent 26/29/34 -> farm rate MASSIVELY up; parent-red baseline of this parent is 0-2-1/0-3), vs parent blue 3-0 (9094 ticks, faster than 12001-13916); vs Richard v135 red 0-3 (4618 ticks, deaths 10 vs 1, lvl 13 vs 18 -> faster collapse), blue **0-3** (5742, lvl 20 vs 23; parent 3-0 -> REGRESSION); 0 INVALID; 3 streams/colour; max instr 15285-19018.
 MIXED / NOT QUALIFIED: mechanism CONFIRMED (idle escort roughly doubles our XP vs our own parent), but idle heroes standing on the creep are punished by Richard's hero aggression on both colours (they stop moving in his engagement range and R2 does not kite). Blue-regression gate fails -> nothing deployed, no membership change.
Next: H-ESCORT-IDLE-02 — same idle hold but only while no enemy hero is within ~8 tiles (reuse R2's existing nearest-enemy-hero distance if in scope; otherwise idle only when mEnemyHeroes==0 in the scan); gate: red must beat 4618 and parent collapse, blue must stay 3-0 vs Richard. No hosted spend until both gates pass.
