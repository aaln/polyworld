...
for scenario in ["start", "on_us", "low_hp", "root", "no_wave", "handoff", "timeout", "returning"]:
  for team in Team:
    for class in [Ranger, Crossbowman, DruidWarden, Warlock, Arcanist, VanguardKnight, DeathKnight, DemonHunter, Berserker, Lich]:
      ...
      if class in [Ranger,Crossbowman]:
        if scenario=="start":doAssert stage==1
        elif scenario=="on_us":doAssert stage==2
        else:doAssert stage==0
      else:
        doAssert stage==0
        if scenario=="start":doAssert vm.runtime.getGlobal("bestKind")==6
...
echo $(%*{"passed":true,"fixtures":rows.len,"rows":rows,
  ...
  "scope":"160constructed release62 host-decision fixtures. Neutral/wave positions and aggro are controlled; complete native and hosted games provide outcome evidence."})
