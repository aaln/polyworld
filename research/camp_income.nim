...
for scenario in ["start"]:
  for team in Team:
    for class in [Ranger, Crossbowman, DruidWarden, Warlock, Arcanist, VanguardKnight, DeathKnight, DemonHunter, Berserker, Lich]:
      ...
      let initialHp=h.hp
      var neutralXp, neutralKills: int
      for tick in 0..<90*TickRate:
        g.tickWorld(proc() = g.runBotDecisions())
        doAssert not vm.failed,vm.lastError
        maxInstructions=max(maxInstructions,vm.lastInstructions)
        maxWork=max(maxWork,vm.lastWork)
        for e in g.world.events:
          if e.kind==XpGained and e.actor.kind==6 and e.target.player==slot:
            neutralXp+=e.amount.int
          if e.kind==Death and e.target.kind==6 and e.actor.player==slot:
            inc neutralKills
      rows.add(%*{"team":team.ord,"class": $class,"initial_hp":initialHp,
        "hp":h.hp,"deaths":h.deaths,"neutral_xp":neutralXp,"neutral_kills":neutralKills,
        "all_xp":h.totalXp,"gold":h.gold,"level":h.level,
        "commands":g.recorder.data.actions.len-before})
...
echo $(%*{"passed":true,"fixtures":rows.len,"rows":rows,
  ...
  "scope":"20controlled90second tier1camp encounters, tenclasses bothsides, level4 fullhealth nearcamp/alliedwave. Actual responsive simulation and XP receipts, not full league score evidence."})
