...
<div id="data"></div><p>Partial results are provisional. All ten side/seat contexts; previous policy and weak-neutral changes compared against deployed source.</p>
<script>async function refresh(){let rows=await(await fetch('/data')).json();let t='<table><tr><th>Arm</th><th>Audited games</th><th>Mean score</th><th>State</th></tr>';for(let r of rows)t+=`<tr><td>${r.arm}</td><td>${r.n}/60</td><td>${r.mean===null?'—':r.mean.toFixed(1)}</td><td>${r.status}</td></tr>`;document.getElementById('data').innerHTML=t+'</table>';}refresh();setInterval(refresh,5000);</script>'''
...
def data():
    base = [r for p in (RAW/'hosted').glob('*/result.json') for r in read(p,[])]
    rows = [('baseline',base,'complete' if len(base)==60 else 'collecting')]
...
if __name__=='__main__':ThreadingHTTPServer(('127.0.0.1',8797),Handler).serve_forever()
